...
export function ProjectSitesPage() {
  ...
  const filteredSites = useMemo(() => {
    ...
    return sites.filter((site) => `${site.name} ${site.location || ""} ${site.streetAddress || ""} ${(site as any).buildingLabel || ""} ${(site as any).floorLabel || ""} ${site.siteCode || ""} ${site.defaultAddressBlock || ""}`.toLowerCase().includes(needle));
  }, [sites, siteQuery]);
  ...
  return (
    <section style={{ display: "grid", gap: 18 }}>
      <SectionHeader
        title="Sites"
        description="Manage site names, locations, street addresses, building and floor labels, codes, and default address blocks in a more operational workspace."
        actions={
      ...
      <div className="panel">
        <div className="toolbar-row" style={{ marginBottom: 12, justifyContent: "space-between" }}>
          <input
            placeholder="Search site name, location, address, building, floor, code, or address block"
            value={siteQuery}
