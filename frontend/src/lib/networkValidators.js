...
export function classifySegmentRole(input) {
    ...
    if (text.includes("printer") || text.includes("print"))
        return "PRINTER";
    if (text.includes("iot") || text.includes("medical") || text.includes("lab") || text.includes("ot"))
        return "IOT";
...
export function usableHostsForPrefix(prefix, role = "OTHER") {
    if (prefix === 32)
...
export function parsePrefixFromCidr(cidr) {
    if (!isValidCidr(cidr))
        return null;
...
export function recommendedPrefixForHosts(hosts, role = "OTHER") {
    ...
    for (let prefix = 30; prefix >= 1; prefix -= 1) {
        if (usableHostsForPrefix(prefix, role) >= target)
            return prefix;
    }
    return 1;
}
export function planningHintForHosts(hosts, role = "OTHER") {
    if (typeof hosts !== "number" || Number.isNaN(hosts) || hosts <= 0)
        return null;
...
export function utilizationForCidr(cidr, estimatedHosts, role = "OTHER") {
    const parsed = parseCidrRange(cidr);
    if (!parsed || typeof estimatedHosts !== "number" || estimatedHosts <= 0)
        return null;
...
export function canonicalizeCidr(cidr) {
    const parsed = parseCidrRange(cidr);
    if (!parsed)
        return null;
...
export function firstUsableIp(parsed, role = "OTHER") {
    if (parsed.prefix === 32)
        return role === "LOOPBACK" ? intToIpv4(parsed.network) : null;
    if (parsed.prefix === 31)
        return role === "WAN_TRANSIT" ? intToIpv4(parsed.network) : null;
    if (parsed.prefix >= 31)
        return null;
    return intToIpv4(parsed.network + 1);
}
export function lastUsableIp(parsed, role = "OTHER") {
    if (parsed.prefix === 32)
        return role === "LOOPBACK" ? intToIpv4(parsed.network) : null;
    if (parsed.prefix === 31)
        return role === "WAN_TRANSIT" ? intToIpv4(parsed.broadcast) : null;
    if (parsed.prefix >= 31)
        return null;
    return intToIpv4(parsed.broadcast - 1);
}
export function subnetFacts(cidr, role = "OTHER") {
    const parsed = parseCidrRange(cidr);
    if (!parsed)
        return null;
...
export function subnetWithinBlock(subnetCidr, parentBlockCidr) {
...
export function suggestSubnetWithinBlock(siteBlockCidr, existingSubnetCidrs = [], estimatedHosts, role = "OTHER") {
    if (!siteBlockCidr || typeof estimatedHosts !== "number" || estimatedHosts <= 0)
        return null;
    const siteBlock = parseCidrRange(siteBlockCidr);
    if (!siteBlock)
        return null;
    const prefix = recommendedPrefixForHosts(estimatedHosts, role);
    if (prefix < siteBlock.prefix)
        return null;
    const step = 2 ** (32 - prefix);
    const existing = existingSubnetCidrs.map(parseCidrRange).filter((item) => Boolean(item));
