import { buildUnifiedDesignTruthModel } from "./designTruthModel";
import { parseDiscoveryWorkspaceState } from "./discoveryFoundation";
import { canonicalizeCidr, cidrOverlap, classifySegmentRole, intToIpv4, ipv4ToInt, parseCidrRange, recommendedPrefixForHosts, subnetFacts, subnetWithinBlock, } from "./networkValidators";
function toNumber(value, fallback = 0) {
    const parsed = Number(value);
    return Number.isFinite(parsed) ? parsed : fallback;
}
function clampPositive(value, floor = 0) {
...
function blockAddressCount(prefix) {
    return 2 ** (32 - prefix);
}
function prefixForAddressCount(addressCount) {
    const required = Math.max(4, Math.ceil(addressCount));
    for (let prefix = 32; prefix >= 0; prefix -= 1) {
        if (blockAddressCount(prefix) >= required)
            return prefix;
    }
    return 0;
}
function cidrAddressCount(cidr) {
    const parsed = cidr ? parseCidrRange(cidr) : null;
    return parsed ? blockAddressCount(parsed.prefix) : 0;
}
function inferWorkingOrganizationBlock(project, siteCount = 1, usersPerSite = 50) {
    const explicit = canonicalizeCidr(project?.basePrivateRange || "");
    if (explicit)
        return { cidr: explicit, assumed: false };
    const roughUsers = Math.max(1, siteCount) * Math.max(25, usersPerSite);
    if (siteCount <= 4 && roughUsers <= 500)
        return { cidr: "10.100.0.0/16", assumed: true };
    if (siteCount <= 16 && roughUsers <= 4000)
        return { cidr: "10.64.0.0/12", assumed: true };
    return { cidr: "10.0.0.0/8", assumed: true };
}
function defaultSiteCode(index) {
    return `SITE${String(index + 1).padStart(2, "0")}`;
}
function siteDisplayName(index, existingCount) {
    if (existingCount === 0 && index === 0)
        return "Primary Site";
    return `Site ${index + 1}`;
}
function buildWorkingSites(sites, plannedSiteCount) {
...
function buildRecommendedSegments(profile, siteIndex, siteCount) {
...
function demandForSite(profile, siteIndex, siteCount) {
...
function findAvailableChildBlock(parent, requestedPrefix, occupied) {
    if (requestedPrefix < parent.prefix)
        return null;
...
function findAvailableChildBlockFromEnd(parent, requestedPrefix, occupied) {
    if (requestedPrefix < parent.prefix)
        return null;
    const step = blockAddressCount(requestedPrefix);
...
function buildConfiguredRow(vlan, siteBlockCidr) {
    ...
    const notes = [];
    if (siteBlockCidr && insideSiteBlock === false)
        notes.push("Configured subnet sits outside the current site summary block.");
    if (estimatedHosts > usableHosts && usableHosts > 0)
        notes.push("Estimated host demand exceeds the usable capacity of this subnet.");
...
function configuredRowsNeedAdjustment(configuredRows, template) {
    if (configuredRows.length === 0)
        return true;
    return configuredRows.some((row) => (row.usableHosts < template.estimatedHosts
        || row.insideSiteBlock === false
        || row.dhcpEnabled !== template.dhcpEnabled));
}
function transitEndpoints(block) {
    if (block.prefix === 31) {
...
function buildWanLinks(input) {
    ...
    if (!shouldPlanWan || !organizationRange || siteHierarchy.length === 0) {
        return { wanReserveBlock: undefined, wanLinks: [] };
    }
    ...
    const wanLinks = targets.map((target, index) => {
        ...
        if (profile.dualIsp)
            notes.push("The requirements call for resilience, so evaluate whether this site also needs a secondary transport path.");
        if (!target.id.startsWith("cloud"))
            notes.push(`Summarize ${target.name} behind ${target.name}'s site block once implementation routing is planned.`);
        return {
            id: `wan-${index + 1}`,
            linkName: `${primarySite.name} ↔ ${target.name}`,
            source: "proposed",
...
function buildRoutingPlan(siteHierarchy, rows, wanLinks, profile) {
    return siteHierarchy.map((site, index) => {
        ...
        const notes = [];
        if (index === 0 && siteHierarchy.length > 1)
            notes.push("Primary site is acting as the initial routing hub for branch summarization and shared services.");
        if (index > 0 && transitAdjacencyCount === 0 && siteHierarchy.length > 1)
            notes.push("Branch site does not yet have a proposed WAN transit link.");
        if (loopback?.subnetCidr)
            notes.push(`Use ${loopback.subnetCidr} as the stable routing and monitoring identity for ${site.name}.`);
        else
            notes.push("Loopback identity is still missing for this site.");
        if (/central/i.test(profile.serverPlacement || "") && index > 0)
            notes.push("This site is expected to consume centralized services over the WAN rather than host the main server block locally.");
        return {
            siteId: site.id,
            siteName: site.name,
...
function buildQosPlan(profile, segmentModel) {
...
function buildLogicalDomains(segmentModel, profile) {
    const grouped = (roles) => segmentModel
...
function buildSecurityZones(logicalDomains, segmentModel, profile) {
    const zones = [];
    const findDomain = (name) => logicalDomains.find((domain) => domain.name === name);
...
function buildSecurityControls(profile, segmentModel) {
    const controls = [
        {
...
function buildSecurityPolicyMatrix(zones) {
    const matrix = [];
    const has = (name) => zones.some((zone) => zone.zoneName === name);
    const add = (row) => matrix.push(row);
...
function buildSegmentationReview(input) {
    ...
    const items = [];
    const hasRole = (role) => segmentModel.some((segment) => segment.role === role);
    const hasZone = (zoneName) => securityZones.some((zone) => zone.zoneName === zoneName);
...
function buildLowLevelDesign(input) {
    ...
    return siteHierarchy.map((site, index) => {
        ...
        if (!site.siteBlockCidr)
            implementationFocus.push("Lock the site summary block before finalizing any child segment subnets.");
        if (siteRows.some((row) => row.source === "proposed"))
            implementationFocus.push("Turn the remaining proposed segments into approved site/VLAN records or intentionally reject them.");
        if (siteRows.some((row) => row.insideSiteBlock === false))
            implementationFocus.push("Move any out-of-block subnets back into the site hierarchy before routing policy is finalized.");
        if (!routing?.loopbackCidr)
            implementationFocus.push("Reserve or confirm a loopback identity so routing, monitoring, and management references stay stable.");
        if (siteHierarchy.length > 1 && transitLinks.length === 0)
            implementationFocus.push("Confirm the site-edge transit adjacency and its routing handoff back toward the hub or shared edge.");
        if (implementationFocus.length === 0)
            implementationFocus.push("Carry the synthesized site summary, routed segments, and trust boundaries into the implementation handoff package.");
        const notes = [
...
function enrichLowLevelDesignWithTruth(input) {
    ...
    return lowLevelDesign.map((site) => {
        ...
        const sourcePriority = ["saved-design", "discovery-derived", "planner-preview", "inferred"];
        const strongestSource = sourcePriority.find((source) => routeSources.includes(source) || boundarySources.includes(source));
        const boundaryNames = Array.from(new Set(designTruthModel.boundaryDomains
            .filter((item) => item.siteId === site.siteId || item.siteName === site.siteName)
            .map((item) => item.boundaryName))).slice(0, 6);
        const serviceNames = Array.from(new Set(designTruthModel.serviceDomains
            .filter((item) => item.siteId === site.siteId || item.siteName === site.siteName || item.consumerSites.includes(site.siteName))
            .map((item) => item.serviceName))).slice(0, 6);
        const flowNames = Array.from(new Set(designTruthModel.flowContracts
            ...
            .map((item) => item.flowLabel))).slice(0, 6);
...
function traceabilityItems(profile, synthesis) {
    const items = [
        {
...
function buildSiteHierarchy(sites, rows) {
...
function buildSegmentModel(rows) {
    return Array.from(rows.reduce((map, row) => {
        ...
        if (roleSortWeight(a.role) !== roleSortWeight(b.role))
...
function buildOpenIssues(rows, siteHierarchy, review, wanLinks, routingPlan, segmentationReview) {
...
function inferTopologyBlueprint(input) {
    ...
    const primarySite = siteHierarchy[0];
    let topologyType = "collapsed-core";
    if (cloudConnected)
        topologyType = "hybrid-cloud";
    else if (siteHierarchy.length > 1 && centralizedBreakout)
        topologyType = "hub-spoke";
    else if (siteHierarchy.length > 1)
        topologyType = "multi-site";
...
function firstSubnetForRole(rows, role) {
    return rows.find((row) => row.role === role && row.subnetCidr !== "Unassigned");
}
function findDmzServerRow(rows, siteId) {
...
function routeBehaviorForTopology(topology, sourceSite, destinationSite) {
    if (topology.topologyType === "collapsed-core")
        return "Local routed core to perimeter edge";
    ...
    if (topology.topologyType === "hybrid-cloud")
        return "On-prem routing domain with controlled cloud-edge advertisement and filtering";
    return "Peer multi-site routing with explicit transport boundaries and site summaries";
}
function buildInterfaceLabels(deviceType, siteRows, options) {
    const zoneRows = (role) => siteRows.filter((row) => row.role === role).map((row) => row.subnetCidr);
    ...
    if (deviceType === "firewall") {
        ...
        if (zoneRows('SERVER').length > 0 || siteRows.some((row) => /dmz/i.test(`${row.segmentName} ${row.purpose}`)))
            labels.push('Gi0/2 dmz / published services');
        if (zoneRows('MANAGEMENT').length > 0)
            labels.push('Gi0/3 management / admin');
        if (topologyType === 'hybrid-cloud' && options?.isPrimary)
            labels.push('Gi0/4 cloud edge transit');
    }
    else if (deviceType === "router") {
        labels.push(topologyType === 'hub-spoke' ? 'Gi0/0 wan / hub transport' : 'Gi0/0 wan / provider');
        labels.push('Gi0/1 lan handoff');
        if (options?.isPrimary || topologyType !== 'collapsed-core')
            labels.push('Lo0 routing identity');
        if (siteRows.some((row) => row.role === 'WAN_TRANSIT') || topologyType === 'hybrid-cloud')
            labels.push(topologyType === 'hybrid-cloud' ? 'Tunnel0 / cloud transport' : 'Tunnel0 / routed transport');
    }
        ...
        if (zoneRows('GUEST').length > 0)
            labels.push('Gi1/0/25-32 guest / AP edge');
    }
    else if (deviceType === "access-point") {
        labels.push('Eth0 trunk / PoE');
        labels.push('SSID corp');
        if (zoneRows('GUEST').length > 0)
            labels.push('SSID guest');
    }
    else if (deviceType === "server") {
        labels.push('Eth0 service / app');
        if (siteRows.some((row) => /dmz/i.test(`${row.segmentName} ${row.purpose}`)))
            labels.push('Eth1 dmz / published');
        if (topologyType === 'hybrid-cloud')
            labels.push('Eth2 sync / cloud peer');
    }
...
function siteToken(input) {
    ...
    if (preference.includes('full location') || preference.includes('full site name'))
        return (fullName || cleanedCode || 'SITE').slice(0, 18);
    if (preference.includes('building') && preference.includes('floor'))
        return [base, buildingToken || 'BLDG', floorToken || 'FLR'].filter(Boolean).join('_').slice(0, 28);
    if (preference.includes('building'))
        return [base, buildingToken || 'BLDG'].filter(Boolean).join('_').slice(0, 24);
    if (preference.includes('floor'))
        return [base, floorToken || 'FLR'].filter(Boolean).join('_').slice(0, 24);
    if (preference.includes('site code'))
        return (cleanedCode || abbreviatedName || 'SITE').slice(0, 12);
    if (cleanedCode)
        return cleanedCode;
    return (abbreviatedName || fullName || 'SITE').slice(0, 18);
}
function devicePrefix(deviceType) {
    switch (deviceType) {
...
function readableDeviceLabel(deviceType, input) {
    switch (deviceType) {
...
function deviceNameForPlacement(input) {
    ...
    if (/short-code/.test(convention))
        return `${prefix}_${token}_${String(sequence).padStart(2, '0')}`;
    if (/location-role-index/.test(convention))
        return `${token}-${prefix}-${String(sequence).padStart(2, '0')}`;
    if (/readable/.test(convention))
        return `${siteName}-${prefix}${sequence}`;
    return `${siteName} ${readable}`;
}
function buildSitePlacements(input) {
    ...
    siteHierarchy.forEach((site, index) => {
        ...
        devices.push({
            ...
            siteId: site.id,
            siteName: site.name,
            deviceName: deviceNameForPlacement({ siteName: site.name, siteCode: site.siteCode, buildingLabel: site.buildingLabel, floorLabel: site.floorLabel, deviceType: edgeDeviceType, isPrimary, topologyType: topology.topologyType, namingConvention: profile.deviceNamingConvention, namingTokenPreference: profile.namingTokenPreference, customNamingPattern: profile.customNamingPattern, sequence: 1 }),
        ...
        devices.push({
            ...
            siteId: site.id,
            siteName: site.name,
            deviceName: deviceNameForPlacement({ siteName: site.name, siteCode: site.siteCode, buildingLabel: site.buildingLabel, floorLabel: site.floorLabel, deviceType: switchingDeviceType, isPrimary, topologyType: topology.topologyType, namingConvention: profile.deviceNamingConvention, namingTokenPreference: profile.namingTokenPreference, customNamingPattern: profile.customNamingPattern, sequence: 1 }),
            siteTier: isPrimary ? (topology.topologyType === "collapsed-core" ? "single-site" : "primary") : "branch",
        ...
        if (profile.wireless || profile.guestWifi) {
            devices.push({
                ...
                siteId: site.id,
                siteName: site.name,
                deviceName: deviceNameForPlacement({ siteName: site.name, siteCode: site.siteCode, buildingLabel: site.buildingLabel, floorLabel: site.floorLabel, deviceType: "access-point", isPrimary, topologyType: topology.topologyType, namingConvention: profile.deviceNamingConvention, namingTokenPreference: profile.namingTokenPreference, customNamingPattern: profile.customNamingPattern, sequence: 1 }),
                siteTier: isPrimary ? (topology.topologyType === "collapsed-core" ? "single-site" : "primary") : "branch",
        ...
        if (serverRows.length > 0) {
            devices.push({
                ...
                siteId: site.id,
                siteName: site.name,
                deviceName: deviceNameForPlacement({ siteName: site.name, siteCode: site.siteCode, buildingLabel: site.buildingLabel, floorLabel: site.floorLabel, deviceType: "server", isPrimary, topologyType: topology.topologyType, namingConvention: profile.deviceNamingConvention, namingTokenPreference: profile.namingTokenPreference, customNamingPattern: profile.customNamingPattern, sequence: 1 }),
                siteTier: isPrimary ? (topology.topologyType === "collapsed-core" ? "single-site" : "primary") : "branch",
                uplinkTarget: switchingDeviceType === "core-switch" ? `${site.name} core switch` : `${site.name} access switch`,
                deviceType: "server",
...
function buildServicePlacements(input) {
    const { profile, topology, siteHierarchy, rows } = input;
    ...
    if (centralServerRow) {
        items.push({
            ...
            attachedDevice: deviceNameForPlacement({ siteName, siteCode: primarySite?.siteCode, buildingLabel: primarySite?.buildingLabel, floorLabel: primarySite?.floorLabel, deviceType: 'core-switch', isPrimary: true, topologyType: topology.topologyType, namingConvention: profile.deviceNamingConvention, namingTokenPreference: profile.namingTokenPreference, customNamingPattern: profile.customNamingPattern, sequence: 1 }),
            upstreamDevice: deviceNameForPlacement({ siteName, siteCode: primarySite?.siteCode, buildingLabel: primarySite?.buildingLabel, floorLabel: primarySite?.floorLabel, deviceType: 'firewall', isPrimary: true, topologyType: topology.topologyType, namingConvention: profile.deviceNamingConvention, namingTokenPreference: profile.namingTokenPreference, customNamingPattern: profile.customNamingPattern, sequence: 1 }),
...
function buildSecurityBoundaries(input) {
...
function addUniqueFlow(target, flow) {
    if (target.some((item) => item.id === flow.id || item.flowLabel === flow.flowLabel))
        return;
    target.push(flow);
}
function wanLinkNameForSite(wanLinks, siteName) {
    if (!siteName)
        return 'Inter-site transport';
    return wanLinks.find((link) => link.endpointASiteName === siteName || link.endpointBSiteName === siteName)?.linkName || 'Inter-site transport';
}
function buildTrafficFlows(input) {
...
function buildFlowCoverage(input) {
    const { profile, topology, siteHierarchy, rows, trafficFlows, servicePlacements } = input;
    const hasRole = (role) => rows.some((row) => row.role === role && row.subnetCidr !== 'Unassigned');
    const multiSite = siteHierarchy.length > 1;
...
export function synthesizeLogicalDesign(project, sites, vlans, profile) {
    ...
    const occupiedSiteBlocks = [];
    const siteDemandMap = new Map();
    workingSites.forEach((site, index) => {
        ...
        if (existingBlock)
            occupiedSiteBlocks.push(existingBlock);
    });
    if (/consistent|summar/i.test(profile.siteBlockStrategy || "") && workingSites.length > 1) {
        ...
        workingSites.forEach((site, index) => {
            if (index === 0 && /central/i.test(profile.serverPlacement || ""))
                return;
    ...
    if (organizationRange) {
        workingSites.forEach((site) => {
            ...
            if (block) {
                ...
                if (!site.note)
                    site.note = "Proposed site block allocated from the organization range.";
            }
            else if (!site.note) {
    ...
    rows.sort((a, b) => {
        ...
        if (siteCompare !== 0)
            return siteCompare;
        if ((a.vlanId || 9999) !== (b.vlanId || 9999))
            return (a.vlanId || 9999) - (b.vlanId || 9999);
        return roleSortWeight(a.role) - roleSortWeight(b.role);
    });
    ...
    const designReview = [
        ...
        ...(segmentationReview.filter((item) => item.severity !== "info").map((item) => ({
            kind: item.severity === "critical" ? "risk" : "assumption",
            title: item.title,
            detail: item.detail,
        }))),
