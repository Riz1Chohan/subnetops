import type {
  ...
  TrafficFlowPath,
  WanLinkPlanRow,
} from "./designSynthesis";
import type { RequirementsProfile } from "./requirementsProfile";
import type { DiscoveryWorkspaceState } from "./discoveryFoundation";
...
export type DesignTruthAuthoritySource = "saved-design" | "discovery-derived" | "planner-preview" | "inferred";
...
interface BuildUnifiedDesignTruthModelInput {
  discoveryState?: DiscoveryWorkspaceState;
...
function boundaryLookupKey(siteName: string, zoneName: string) {
  return `${siteName}::${zoneName}`.toLowerCase();
}

function firstMatchingBoundaryId(input: { siteName?: string; zoneName?: string; boundaries: DesignTruthBoundaryDomain[] }) {
  ...
  if (zoneMatches.length === 1) return zoneMatches[0]?.id;
  return undefined;
}

function summarizeTopologyRole(input: { site: SiteHierarchyItem; topology: TopologyBlueprint }) {
  const { site, topology } = input;
  if (topology.topologyType === "collapsed-core") return "Single-site routed domain";
  if (site.id === topology.primarySiteId) return topology.cloudConnected ? "Primary site and shared-service/cloud control point" : "Primary site and shared-service control point";
  if (topology.topologyType === "hub-spoke") return `Spoke site anchored toward ${topology.primarySiteName || "primary site"}`;
  if (topology.topologyType === "hybrid-cloud") return "On-prem site participating in hybrid route domain";
...
function inferBoundaryPlacement(input: {
  ...
  sitePlacements: SitePlacementDevice[];
  servicePlacements: ServicePlacementItem[];
  placementByName: Map<string, SitePlacementDevice>;
}) {
  ...
  if (zonePlacement) return zonePlacement;

  const preferredBoundaryPlacement = sitePlacements.find(
    (placement) =>
      placement.siteName === siteName
      && ["firewall", "router", "core-switch", "distribution-switch", "cloud-edge"].includes(placement.deviceType),
  );
  if (preferredBoundaryPlacement) return preferredBoundaryPlacement;

  return sitePlacements.find((placement) => placement.siteName === siteName);
}

function inferUpstreamBoundary(input: { siteName: string; topology: TopologyBlueprint }) {
  const { siteName, topology } = input;
  if (topology.internetBreakout === "distributed") return "Local internet / upstream edge";
  if (topology.primarySiteName && topology.primarySiteName !== siteName) return `${topology.primarySiteName} upstream boundary`;
  return "Shared edge boundary";
}

function inferInboundPolicy(zoneName: string) {
  const text = zoneName.toLowerCase();
  if (text.includes("guest")) return "Default deny to trusted zones; allow reviewed internet egress and return traffic only.";
  if (text.includes("dmz") || text.includes("public")) return "Default deny inbound except explicitly published application paths.";
  if (text.includes("management")) return "Allow only reviewed administrative sources with MFA, logging, and jump-host discipline.";
  return "Default deny inbound except approved service, management, or dependency-specific access.";
}

function inferEastWestPolicy(zoneName: string) {
  const text = zoneName.toLowerCase();
  if (text.includes("management")) return "Restrict lateral movement to reviewed administration paths only.";
  if (text.includes("server") || text.includes("service") || text.includes("dmz")) return "Permit only dependency-driven service paths; block broad east-west trust by default.";
  return "Block broad east-west trust and allow only reviewed inter-zone business flows.";
}

function inferNatPolicy(zoneName: string) {
  const text = zoneName.toLowerCase();
  if (text.includes("guest") || text.includes("internet")) return "Apply NAT only at the north-south edge for internet-facing egress.";
  if (text.includes("dmz") || text.includes("public")) return "Review static or policy NAT only for explicitly published services.";
...
function zoneEvidenceTokens(zoneName: string) {
  const base = normalizeText(zoneName);
  const tokens = new Set<string>([base]);
  if (base.includes("guest")) { tokens.add("guest"); tokens.add("wifi"); tokens.add("wireless"); }
  if (base.includes("manage")) { tokens.add("management"); tokens.add("admin"); tokens.add("monitor"); }
  if (base.includes("server") || base.includes("service")) { tokens.add("server"); tokens.add("service"); }
  if (base.includes("dmz") || base.includes("public")) { tokens.add("dmz"); tokens.add("public"); tokens.add("published"); }
  if (base.includes("voice")) { tokens.add("voice"); tokens.add("phone"); }
  if (base.includes("printer")) tokens.add("printer");
  if (base.includes("iot") || base.includes("ot")) { tokens.add("iot"); tokens.add("ot"); }
  if (base.includes("camera")) tokens.add("camera");
  if (base.includes("user") || base.includes("staff")) { tokens.add("user"); tokens.add("staff"); }
  return [...tokens].filter(Boolean);
}

function buildSiteSearchTokens(siteName: string, siteCode?: string) {
  const tokens = [normalizeText(siteName)];
  if (siteCode) tokens.push(normalizeText(siteCode));
  return tokens.filter(Boolean);
}

function hasDiscoverySiteEvidence(input: { siteName: string; siteCode?: string; discoveryState?: DiscoveryWorkspaceState; kind: "route" | "boundary"; zoneName?: string }) {
  const { siteName, siteCode, discoveryState, kind, zoneName } = input;
  if (!discoveryState) return false;
  ...
  if (!siteMatched && siteTokens.length > 1) return false;
  ...
  if (zoneTokens.length === 0) return siteMatched;
  return zoneTokens.some((token) => corpus.includes(token));
}

function plannerPreviewSupportsBoundary(input: { zoneName: string; profile: RequirementsProfile; segmentCount: number; serviceCount: number }) {
  const { zoneName, profile, segmentCount, serviceCount } = input;
  const text = normalizeText(zoneName);
  if (segmentCount > 0 || serviceCount > 0) return true;
  if (text.includes("guest")) return profile.guestWifi || profile.wireless;
  if (text.includes("manage")) return profile.management;
  if (text.includes("voice")) return profile.voice;
  if (text.includes("printer")) return profile.printers;
  if (text.includes("iot") || text.includes("ot")) return profile.iot;
  if (text.includes("camera")) return profile.cameras;
  if (text.includes("dmz") || text.includes("public")) return profile.cloudConnected || profile.remoteAccess || normalizeText(profile.serverPlacement).includes("dmz");
  return true;
}

export function buildUnifiedDesignTruthModel(input: BuildUnifiedDesignTruthModelInput): UnifiedDesignTruthModel {
  const {
    discoveryState,
    profile,
    topology,
  ...
  const routeDomains: DesignTruthRouteDomain[] = routingPlan.map((route) => {
    ...
    return {
      ...
      sourceModel: "explicit" as const,
      authoritySource: "saved-design" as const,
  ...
  const inferredRouteDomains: DesignTruthRouteDomain[] = siteHierarchy
    ...
    .map((site) => {
      ...
      const authoritySource = hasDiscoveryRouteEvidence
        ? "discovery-derived" as const
        : hasPlannerPreviewRouteEvidence
          ? "planner-preview" as const
          : "inferred" as const;
      ...
      return {
        ...
        sourceModel: authoritySource === "inferred" ? "inferred" as const : "explicit" as const,
        authoritySource,
  ...
  const boundaryDomains: DesignTruthBoundaryDomain[] = securityBoundaries.map((boundary, index) => {
  ...
  const inferredBoundaryDomains: DesignTruthBoundaryDomain[] = inferredBoundarySeeds
    .map((seed) => {
  ...
  const serviceDomains: DesignTruthServiceDomain[] = servicePlacements.map((service) => {
  ...
  const wanAdjacencies: DesignTruthWanAdjacency[] = wanLinks.map((link) => {
    ...
    if (!endpointARouteDomain) unresolvedReferences.push(`${link.linkName}: no route domain found for ${link.endpointASiteName}.`);
    if (!endpointBRouteDomain) unresolvedReferences.push(`${link.linkName}: no route domain found for ${link.endpointBSiteName}.`);
  ...
  const flowContracts: DesignTruthFlowContract[] = trafficFlows.map((flow) => {
    ...
    const serviceIds = serviceDomains
      .filter(
        (service) =>
          service.serviceName === flow.source
    ...
    const wanAdjacencyIds = wanAdjacencies
      .filter(
        (adjacency) =>
          flow.path.some((hop) => hop.includes(adjacency.endpointASiteName) || hop.includes(adjacency.endpointBSiteName))
    ...
    if (flow.sourceSite && !sourceRouteDomain) localUnresolved.push(`Missing route domain for source site ${flow.sourceSite}`);
    if (flow.destinationSite && !destinationRouteDomain) localUnresolved.push(`Missing route domain for destination site ${flow.destinationSite}`);
    if (!boundaryIds.length && (flow.sourceZone || flow.destinationZone)) localUnresolved.push(`No boundary objects linked for ${flow.flowLabel}`);
  ...
  const segmentNodes: DesignTruthSegmentNode[] = addressingPlan.map((row) => {
  ...
  const siteNodes: DesignTruthSiteNode[] = siteHierarchy.map((site) => {
  ...
  const relationshipEdges: DesignTruthRelationshipEdge[] = [];
  ...
  const coverage: DesignTruthCoverageItem[] = [
    {
