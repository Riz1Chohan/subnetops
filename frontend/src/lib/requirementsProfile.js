export const defaultRequirementsProfile = {
    planningFor: "Office",
...
export function parseRequirementsProfile(value) {
    if (!value)
...
export function stringifyRequirementsProfile(value) {
    return JSON.stringify(value);
}
export function buildGuidedPrompt(values) {
    const features = [];
    if (values.guestWifi)
        features.push(`isolated guest Wi-Fi with ${values.guestPolicy}`);
    if (values.voice)
        features.push(`voice VLANs with ${values.voiceQos}`);
    if (values.management)
        features.push(`a management network with ${values.managementAccess}`);
    if (values.printers)
        features.push("printer segmentation");
    if (values.iot)
        features.push("IoT separation");
    if (values.cameras)
        features.push("camera/security device separation");
    if (values.wireless)
        features.push(`wireless access planning using ${values.wirelessModel}`);
    if (values.remoteAccess)
        features.push(`remote access using ${values.remoteAccessMethod}`);
    if (values.dualIsp)
        features.push(`redundancy target of ${values.resilienceTarget}`);
    if (values.cloudConnected || values.environmentType !== "On-prem")
        features.push(`cloud connectivity via ${values.cloudConnectivity} with ${values.cloudProvider}`, `cloud identity boundary of ${values.cloudIdentityBoundary}`, `cloud traffic boundary of ${values.cloudTrafficBoundary}`, `cloud hosting model of ${values.cloudHostingModel}`, `cloud network model of ${values.cloudNetworkModel}`, `cloud routing model of ${values.cloudRoutingModel}`);
    if (values.management || values.guestWifi || values.remoteAccess || values.iot || values.cameras)
        features.push(`security posture of ${values.securityPosture}`, `trust boundary model of ${values.trustBoundaryModel}`, `administrative boundary of ${values.adminBoundary}`, `identity model of ${values.identityModel}`);
    ...
    if (values.customRequirementsNotes.trim())
        features.push(`custom requirements notes: ${values.customRequirementsNotes.trim()}`);
...
function shortNameToken(value, fallback = 'SITE') {
...
function previewSiteToken(site, preference) {
    ...
    if (pref.includes('full location') || pref.includes('full site name'))
        return siteNameToken;
    if (pref.includes('closet'))
        return [siteCodeToken || shortSiteToken, buildingToken, floorToken, closetToken].filter(Boolean).join('_').slice(0, 30);
    if (pref.includes('building') && pref.includes('floor'))
        return [siteCodeToken || shortSiteToken, buildingToken, floorToken].filter(Boolean).join('_').slice(0, 24);
    if (pref.includes('building'))
        return [siteCodeToken || shortSiteToken, buildingToken].filter(Boolean).join('_').slice(0, 20);
    if (pref.includes('floor'))
        return [siteCodeToken || shortSiteToken, floorToken].filter(Boolean).join('_').slice(0, 20);
    if (pref.includes('site code'))
        return (siteCodeToken || shortSiteToken).slice(0, 18);
    return (siteCodeToken || shortSiteToken || siteNameToken).slice(0, 18);
}
function previewDevicePrefix(role) {
    switch (role) {
...
function applyCustomPattern(input) {
...
function previewDeviceName(input) {
    ...
    if (normalizedConvention.includes('short-code'))
        return `${prefix}_${token}_${String(index).padStart(2, '0')}`;
    if (normalizedConvention.includes('location-role-index'))
        return `${token}-${prefix}-${String(index).padStart(2, '0')}`;
    if (normalizedConvention.includes('readable'))
        return `${siteLabel}-${prefix}${index}`;
    return `${siteLabel} ${role.replace(/-/g, ' ')} ${index}`;
}
export function buildNamingPreviewExamples(values, sites) {
...
export function buildProjectSummaryDescription(values, maxLength = 320) {
    const parts = [
    ...
    if (values.guestWifi)
        parts.push(`guest access`);
    if (values.management)
        parts.push(`management network`);
    if (values.voice)
        parts.push(`voice`);
    if (values.remoteAccess)
        parts.push(`remote access`);
    if (values.cloudConnected || values.environmentType !== "On-prem")
        parts.push(`cloud-connected`);
    if (values.dualIsp)
...
export function buildGuidedDescription(values) {
    const details = [];
    if (values.guestWifi)
        details.push(`guest access policy: ${values.guestPolicy}`);
    if (values.management)
        details.push(`management access: ${values.managementAccess}`);
    if (values.voice)
        details.push(`voice treatment: ${values.voiceQos}`);
    if (values.wireless)
        details.push(`wireless model: ${values.wirelessModel}`);
    if (values.remoteAccess)
        details.push(`remote access via ${values.remoteAccessMethod}`);
    if (values.dualIsp)
        details.push(`resilience target: ${values.resilienceTarget}`);
    if (values.cloudConnected || values.environmentType !== "On-prem")
        details.push(`cloud pattern: ${values.cloudProvider} over ${values.cloudConnectivity}`, `cloud identity boundary: ${values.cloudIdentityBoundary}`, `cloud traffic boundary: ${values.cloudTrafficBoundary}`, `cloud hosting model: ${values.cloudHostingModel}`, `cloud network model: ${values.cloudNetworkModel}`, `cloud routing model: ${values.cloudRoutingModel}`);
    if (values.management || values.guestWifi || values.remoteAccess || values.iot || values.cameras)
        details.push(`security posture: ${values.securityPosture}`, `trust boundaries: ${values.trustBoundaryModel}`, `admin boundary: ${values.adminBoundary}`, `identity model: ${values.identityModel}`);
    ...
    if (values.customRequirementsNotes.trim())
        details.push(`custom requirements notes: ${values.customRequirementsNotes.trim()}`);
    return [
...
export function planningTracks(values) {
    const tracks = ["Core requirements", "Addressing"];
    const siteCount = Number(values.siteCount || "0");
    if (values.guestWifi || values.management || values.remoteAccess || values.iot || values.cameras)
        tracks.push("Security");
    if (values.environmentType !== "On-prem" || values.cloudConnected)
        tracks.push("Cloud / Hybrid");
    if (values.wireless || values.guestWifi)
        tracks.push("Wireless");
    if (values.voice || Number(values.phoneCount || "0") > 0)
        tracks.push("Voice / QoS");
    if (siteCount > 1 || values.internetModel !== "internet at each site")
        tracks.push("WAN / Multi-site");
    if (Number(values.cameraCount || "0") > 0 || Number(values.iotDeviceCount || "0") > 0)
        tracks.push("Specialty devices");
    tracks.push("Operations", "Implementation / Handoff");
    return tracks;
}
export function planningTrackStatuses(values) {
    const siteCount = Number(values.siteCount || "0");
...
export function planningReadinessSummary(values) {
    ...
    if (reviewCount === 0 && readyCount > 0)
        completionLabel = "Review-ready";
    else if (readyCount >= 4)
        completionLabel = "Mostly shaped";
    else if (readyCount >= 2)
        completionLabel = "Taking shape";
