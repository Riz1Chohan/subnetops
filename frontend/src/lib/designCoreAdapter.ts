...
function backendImplementationPhases(plan: ImplementationPlanModel, local: ImplementationPhasePlan[]): ImplementationPhasePlan[] {
  ...
  return [...plan.stages]
    ...
    .map((stage) => {
      ...
      return {
        phase: `${stage.sequence}. ${stage.name}`,
...
export function applyDesignCoreSnapshotToDisplayDesign(
  ...
  return {
    ...
    implementationPhases: implementationPlan ? backendImplementationPhases(implementationPlan, localDesign.implementationPhases) : localDesign.implementationPhases,
