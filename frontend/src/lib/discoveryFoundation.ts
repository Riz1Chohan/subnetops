...
export interface DiscoverySummary {
  ...
  siteMentions: number;
  deviceMentions: number;
  routingProtocols: string[];
  securityControls: string[];
  wirelessSignals: string[];
  lifecycleFlags: string[];
  currentStateHighlights: string[];
  gaps: string[];
  constraints: string[];
  ingestionCoverage: Array<{ label: string; complete: boolean; note: string }>;
  inferredRisks: string[];
  suggestedNextInputs: string[];
  migrationComplexity: "Low" | "Moderate" | "High";
}

const storageKey = (projectId: string) => `subnetops.discovery.${projectId}`;

export const emptyDiscoveryWorkspaceState = (): DiscoveryWorkspaceState => ({
  topologyText: "",
  inventoryText: "",
  addressingText: "",
  routingText: "",
...
export function parseDiscoveryWorkspaceState(raw?: string | null): DiscoveryWorkspaceState {
  if (!raw) return emptyDiscoveryWorkspaceState();
  try {
    const parsed = JSON.parse(raw) as Partial<DiscoveryWorkspaceState>;
    return {
...
export function resolveDiscoveryWorkspaceState(projectId: string, project?: Pick<Project, "discoveryJson"> | null): DiscoveryWorkspaceState {
  const fromProject = parseDiscoveryWorkspaceState(project?.discoveryJson);
  if (project?.discoveryJson) return fromProject;
  return loadDiscoveryWorkspaceState(projectId);
}

export function loadDiscoveryWorkspaceState(projectId: string): DiscoveryWorkspaceState {
  if (typeof window === "undefined") return emptyDiscoveryWorkspaceState();
  try {
...
export function saveDiscoveryWorkspaceState(projectId: string, value: DiscoveryWorkspaceState) {
...
export function clearDiscoveryWorkspaceState(projectId: string) {
  if (typeof window === "undefined") return;
...
function keywordMatches(text: string, pattern: RegExp) {
...
function estimateDeviceMentions(text: string) {
...
function detectLifecycleFlags(text: string) {
  const lines = lineItems(text);
...
export function analyzeDiscoveryWorkspaceState(input: {
  ...
  if (filledSections < 3) inferredRisks.push("Discovery coverage is still shallow, so future design outputs may reflect assumptions more than observed current-state facts.");
  if (state.addressingText.trim().length === 0 && vlans.length === 0) inferredRisks.push("No clear current-state addressing baseline exists yet, which makes overlap analysis and migration planning less reliable.");
  if (routingProtocols.length === 0 && state.routingText.trim().length > 0) inferredRisks.push("Routing notes are present but protocol intent is still unclear, which can hide redistribution or summarization risk.");
  if (securityControls.length === 0 && state.securityText.trim().length > 0) inferredRisks.push("Security notes exist but named controls are still vague, so trust-boundary and policy assumptions may be under-modeled.");
  if (lifecycleFlags.length > 0) inferredRisks.push("Legacy or end-of-support signals are present in the current-state inventory, so implementation planning should separate refresh from migration work.");
  if (gaps.length === 0) inferredRisks.push("No explicit pain points were captured, which makes it harder to prove that the new design solves the right problems.");

  const suggestedNextInputs: string[] = [];
  if (state.topologyText.trim().length === 0) suggestedNextInputs.push("Paste the current topology story: sites, WAN type, internet edge, cloud edge, and any critical inter-site paths.");
  if (state.inventoryText.trim().length === 0) suggestedNextInputs.push("Paste a simple current inventory list with device role, platform, software version, and any EoL/EoS notes.");
  if (state.addressingText.trim().length === 0 && vlans.length === 0) suggestedNextInputs.push("Paste the current subnet/VLAN/gateway baseline so SubnetOps can compare existing addressing to the target design.");
  if (state.routingText.trim().length === 0) suggestedNextInputs.push("Capture the current routing and transport posture: OSPF/BGP/static, MPLS/SD-WAN/VPN, default-route behavior, and edge dependencies.");
  if (state.securityText.trim().length === 0) suggestedNextInputs.push("Capture the current security posture: firewalling, NAC/802.1X, VPN, IDS/IPS, logging, and remote-access controls.");
  if (state.gapText.trim().length === 0) suggestedNextInputs.push("List the current pain points: overlap, outages, flat VLANs, weak guest isolation, aging gear, or cloud reachability issues.");

  let migrationComplexity: DiscoverySummary["migrationComplexity"] = "Low";
  ...
  if (complexityScore >= 4) migrationComplexity = "High";
  else if (complexityScore >= 2) migrationComplexity = "Moderate";
