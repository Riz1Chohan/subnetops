...
assert(/^0\.(103|104)\.0$/.test(pkg.version), 'package version must be 0.103.0 or a compatible newer diagram release');
...
assert(canvas.includes('scope === "wan-cloud"') && canvas.includes('const columns = enterprise ? 3'), 'WAN branch grid layout missing');
