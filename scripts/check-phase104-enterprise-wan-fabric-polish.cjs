...
assert(/^0\.(104|105)\.0$/.test(pkg.version), 'package version must be 0.104.0 or compatible newer diagram release');
...
assert(canvas.includes('bounds.width - 230'), 'fabric guide must span the enterprise branch grid');
...
assert(canvas.includes('enterpriseView && (scope === "wan-cloud" || mode === "physical") && count > 1'), 'duplicate enterprise link labels must be suppressed');
assert(canvas.includes('gapY = enterprise ? 320') || canvas.includes('Math.floor(index / columns) * 440'), 'WAN enterprise row spacing must prevent site-card overlap');
assert(canvas.includes('gapY = enterprise ? 300') || canvas.includes('Math.floor(index / columns) * 420'), 'physical enterprise row spacing must prevent site-card overlap');
