...
assert(/^0\.(104|105|106)\.0$/.test(pkg.version), 'package version must be 0.104.0 or compatible newer diagram release');
...
assert(canvas.includes('gapY = enterprise ? 320') || canvas.includes('gapY = enterprise ? 275') || canvas.includes('Math.floor(index / columns) * 440'), 'WAN enterprise row spacing must prevent site-card overlap');
assert(canvas.includes('gapY = enterprise ? 300') || canvas.includes('gapY = enterprise ? 260') || canvas.includes('Math.floor(index / columns) * 420'), 'physical enterprise row spacing must prevent site-card overlap');
