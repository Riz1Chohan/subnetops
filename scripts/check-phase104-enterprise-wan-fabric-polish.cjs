...
assert(pkg.version === '0.104.0', 'package version must be 0.104.0');
...
assert(canvas.includes('Math.floor(index / columns) * 440'), 'WAN enterprise row spacing must prevent site-card overlap');
assert(canvas.includes('Math.floor(index / columns) * 420'), 'physical enterprise row spacing must prevent site-card overlap');
