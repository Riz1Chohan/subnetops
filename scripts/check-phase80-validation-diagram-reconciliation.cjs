...
assertJson('package.json', (pkg) => ['0.80.0','0.81.0','0.82.0','0.83.0'].includes(pkg.version), 'Root package version must be 0.80.0 or compatible successor through Phase 83 for Phase 80');
...
assertJson('package.json', (pkg) => ['0.81.0','0.82.0','0.83.0'].includes(pkg.version) || read('backend/src/services/requirementsRuntimeProof.service.ts').includes('PHASE_80_VALIDATION_DIAGRAM_RECONCILIATION'), 'Health/runtime proof marker must expose Phase 80 or compatible Phase 81');
assertJson('package.json', (pkg) => ['0.81.0','0.82.0','0.83.0'].includes(pkg.version) || read('backend/src/services/requirementsRuntimeProof.service.ts').includes('version: "0.80.0"'), 'Runtime proof marker must expose Phase 80 or compatible Phase 81 version');
