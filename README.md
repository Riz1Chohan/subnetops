...
Reports are for network engineers and project stakeholders reviewing the generated network planning package. They must be professional deliverables, not developer logs. Report content must not include developer commentary, code snippets, repair-phase notes, chat commentary, test-fixture language, debug-dump wording, or internal implementation TODOs.

Allowed report content: design scope, requirements baseline, addressing, segmentation, routing/transport intent, validation/readiness posture, risks/open review items, engineer checklist, and evidence appendix rows written for engineering review.

Developer proof belongs in TypeScript selftests, static kill-switch scripts, and this README. It does not belong in PDF, DOCX, CSV, report-page prose, or diagram labels unless the wording is converted into engineer-facing evidence language.
...
The initial golden-fixture pass freezes the uploaded clinic export as the golden failure case. The captured package has 3 materialized sites, 39 addressing rows, 0 requirement-output gaps, 11 executive-summary root blockers, 174 warnings, a legacy validation rollup that reports 50 root blockers, 543 omitted evidence rows, 39 IPAM candidate allocations, 0 approved IPAM allocations, and blocked implementation readiness.

The fixture exists to prove the root defect cannot be ignored: exported count surfaces must eventually consume one authoritative validation ledger. The report layer must not reclassify findings, promoted propagated blockers must not inflate root blocker counts, and candidate IPAM rows must remain review-required instead of being called approved source-of-truth authority.

Guard and selftest:

```bash
node scripts/check-clinic-golden-fixture.cjs
...
True blockers are limited to missing referenced policy/NAT zones, unresolved NAT zone references, and direct modeled policy contradictions such as a flow expecting deny while the modeled first-match action allows the same path. Required NAT without concrete translation remains review-required unless the NAT rule references a missing zone. Shadowed and overbroad rules are review debt unless a direct contradiction is proven by the modeled flow evidence.

Required behavior:

- Security-policy findings carry a review class: `STRUCTURAL_BLOCKER`, `DIRECT_POLICY_CONFLICT_BLOCKER`, `PLANNING_REVIEW_ITEM`, or `ADVISORY_ITEM`.
- `SECURITY_FLOW_POLICY_MISSING`, `SECURITY_FLOW_NAT_MISSING`, `SECURITY_NAT_REQUIRED_FLOW_UNCOVERED`, `SECURITY_NAT_RULE_WITHOUT_COVERED_FLOW`, `SECURITY_IMPLICIT_DENY_NOT_MODELED`, `SECURITY_BROAD_PERMIT_TO_TRUSTED_ZONE`, `SECURITY_RULE_SHADOWED_BY_EARLIER_RULE`, and `SECURITY_LOGGING_EVIDENCE_GAP` are planning review items.
- `SECURITY_POLICY_SOURCE_ZONE_MISSING`, `SECURITY_POLICY_DESTINATION_ZONE_MISSING`, `SECURITY_NAT_SOURCE_ZONE_MISSING`, and `SECURITY_NAT_DESTINATION_ZONE_MISSING` are structural blockers.
- `SECURITY_FLOW_POLICY_CONFLICT`, `SECURITY_DEFAULT_DENY_WEAKENED_BY_ALLOW`, and `SECURITY_WAN_INBOUND_NOT_DMZ` are direct policy-conflict blockers.
- Reports label missing policy and missing NAT as review, show finding class, and reserve true-blocker counts for structural or direct-contradiction evidence.
- The frontend security page renders backend-provided true blocker count, review finding count, and finding class instead of reclassifying rows locally.

Regression guard: `scripts/check-security-policy-review-classification.cjs`.


### Platform/BOM and discovery state cleanup

Platform/BOM and discovery/current-state readiness now expose explicit backend states instead of repeating vague review warnings. Platform/BOM uses `platformProfileState` with `NOT_STARTED`, `ROLE_BASED_ASSUMPTIONS`, `SAVED_REVIEW_REQUIRED`, and `PROCUREMENT_READY`. Discovery/current-state uses `discoveryState` with `NOT_CAPTURED`, `MANUAL_NOTES_ONLY`, `IMPORTED_REVIEW_REQUIRED`, and `VERIFIED_CURRENT_STATE`.

Required behavior:

- Platform/BOM is not procurement-ready when no saved platform profile exists. Role-based BOM rows remain advisory engineering assumptions, not purchasing authority.
- Saved platform profiles remain `SAVED_REVIEW_REQUIRED` until physical inventory, SKU/model, licensing, support, lifecycle, and procurement evidence are confirmed.
- Discovery is review-required when current-state inventory is not imported. Manual notes can guide tasks, but they cannot become verified current-state authority.
- Imported current-state evidence remains `IMPORTED_REVIEW_REQUIRED` until reconciliation/validation removes review and conflict gates.
- Verified current state is allowed only when validated evidence exists and no V1 conflict or review gate remains.
- Report/export and frontend pages must consume backend `platformProfileState`, `procurementReadinessReason`, `discoveryState`, and `discoveryReadinessReason` instead of generating local status language.
- Professional reports may explain procurement/discovery readiness in engineer-facing terms only. Developer commentary, code, debug wording, and repair notes stay out of reports.

Expected report wording:

- Platform/BOM: not procurement-ready because no saved platform profile exists.
- Discovery: review-required because current-state inventory is not imported.

Regression guard: `node scripts/check-platform-bom-discovery-state-cleanup.cjs`.

### Deployment/API stability hardening

Production deployment now requires explicit API wiring instead of hostname guessing. The production frontend must not infer hostnames or silently fall back to /api. Local development may still use `http://localhost:4000/api`, but production builds must receive `VITE_API_BASE_URL` at build time.

Required behavior:

- The Vite production build fails clearly when `VITE_API_BASE_URL` is missing.
- The frontend API client does not rewrite frontend hostnames into backend hostnames.
- The frontend API client does not use `/api` as a silent production fallback.
- Docker frontend builds require a non-empty `VITE_API_BASE_URL` build argument.
- Render/static hosting config must set frontend `VITE_API_BASE_URL` explicitly.
- Backend production startup requires `CORS_ORIGIN` and `FRONTEND_APP_URL`; missing deployment wiring fails at startup instead of producing broken login/fetch behavior.
- Backend readiness stays on `/api/health/ready` and reports database plus deployment-configuration checks.

Expected deployment values:

- Frontend `VITE_API_BASE_URL`: backend API base URL, for example `https://subnetops-backend.onrender.com/api`.
- Backend `CORS_ORIGIN`: deployed frontend origin, for example `https://subnetops-frontend.onrender.com`.
- Backend `FRONTEND_APP_URL`: deployed frontend URL, for example `https://subnetops-frontend.onrender.com`.

Regression guard: `node scripts/check-deployment-api-stability.cjs`.


### Runtime CI/export consistency kill switches

Runtime export proof now checks that JSON, CSV, PDF, DOCX, and the frontend report surface consume the same canonical `V1ReportExportTruth.evidenceView` root-blocker count. Candidate IPAM rows cannot be labelled as approved authority, blocked implementation posture cannot claim executable readiness, and blocked requirement rows must carry blocker proof.

Required behavior:

- JSON, CSV, PDF, DOCX, and frontend report root-blocker counts derive from `V1ReportExportTruth.evidenceView.validation.rootBlockerCount`.
- Candidate IPAM evidence remains candidate-only and must not be labelled approved authority.
- Implementation-ready claims require the backend evidence view to allow implementation readiness.
- Blocked requirement rows must include `blockedReason` and `lifecycleProofStatus: PROVEN`.
- CSV exports include an export consistency proof row, and the JSON export exposes the evidence view plus consistency proof.
- Professional reports show only engineer-facing consistency evidence, not developer/debug commentary.

Regression guard: `node scripts/check-runtime-export-consistency-kill-switches.cjs`.

### Frontend truth cards / backend evidence alignment

Frontend truth cards now use the shared `V1ReportExportTruth.evidenceView` path through `frontend/src/lib/reportEvidenceView.tsx`. Project Overview, Report, Diagram, Enterprise IPAM, and Implementation pages share the same backend evidence cards instead of recalculating readiness, blocker counts, IPAM authority, or hidden evidence locally.

Required behavior:

- Project Overview, Report, Diagram, Enterprise IPAM, and Implementation pages resolve evidence through `getCanonicalReportEvidenceView(designCore)`.
- Frontend cards consume backend evidence view fields for design readiness, implementation readiness, root blockers, propagated blockers, review items, candidate IPAM, approved IPAM, hidden blocker surfaces, and hidden review surfaces.
- The frontend may format backend values, but it must not recompute these readiness or authority values from validation arrays, local synthesized rows, or display-only models.
- If the backend evidence view is unavailable, the frontend shows an unavailable/review state instead of fabricating replacement counts.
- Report/export, diagram, IPAM, implementation, and overview pages therefore agree on the same backend evidence source.

Regression guard: `node scripts/check-frontend-evidence-view-alignment.cjs`.
...
The final cleanup pass is the last discipline pass for the V1 repair chain. It does not add a new feature surface. It locks the repository shape and report boundary so the repaired truth model does not rot through documentation sprawl, generated-output bloat, or customer-facing report leakage.

Required behavior:

- README.md remains the only Markdown documentation file.
- No phase notes, version notes, docs/doc files, handoff Markdown files, or scattered documentation files may be added.
- No generated build output, dependency folders, coverage output, or report deliverables belong in the source package.
- Professional reports remain engineer-facing deliverables only: no developer commentary, code snippets, repair notes, chat commentary, debug language, internal implementation notes, or proof-dump wording.
- Final proof remains in selftests, static guards, runtime consistency checks, and this README; reports may include only engineer-facing evidence summaries.
- `check:quality` and the regression kill-switch chain must keep all V1 guards wired, including the final cleanup guard.

The final cleanup guard checks the repository itself, not a promise written in prose. It scans for extra Markdown files, build/dependency/coverage directories, generated report artifacts, missing guard wiring, and weakened professional-report boundary hooks.
...
Execution authority requires configured, imported, approved, or discovered source truth. Planned, proposed, materialized, review-required, blocked, or candidate-style rows remain planning candidates or review-required rows instead of executable change steps.

High-risk steps require an operational-safety dependency before they can become execution-ready. Missing rollback, missing verification evidence, missing preconditions, or missing backend source object lineage remain structural blockers.
...
- `ROOT_BLOCKER` is the only class that may count as a true root blocker.
- `PROPAGATED_BLOCKER` stays visible but must not inflate the root blocker count.
- `DERIVED_IMPACT` records secondary consequences without creating a new root cause.
- `REVIEW_ITEM` stays review-required until approved, imported, accepted, or explicitly deferred.
- Export, report, diagram, and frontend consumers must consume the ledger fields instead of reclassifying validation rows from titles, messages, or regex patterns.

Regression guard: `node scripts/check-validation-ledger-authority.cjs`.

### Export validation ledger consumer

This pass removes report/export authority over validation classification. The export service may group findings for professional readability, but it must not promote rows into root blockers by severity, title, message text, or regex. Root blockers are counted only when the persisted validation ledger says `readinessCategory = BLOCKING` and `findingClass = ROOT_BLOCKER`.

Required behavior:

- Root blocker count is calculated through the ledger predicate only.
- Propagated blockers remain visible as propagated blockers and do not inflate root blocker totals.
- Missing ledger taxonomy is surfaced as review-required export evidence instead of guessed from report text.
- Professional report wording uses root-blocker and review/warning language, not generic error-level blocker wording.
- The main report remains engineer-facing only; this implementation proof stays in README/tests, not in report prose.

Regression guard: `node scripts/check-export-validation-ledger-consumer.cjs`.

### Root blocker taxonomy authority

Root blocker classification is now centralized in the validation domain instead of being decided by report/export wording, severity alone, or pattern matching in the readiness control. A root blocker requires both a blocking readiness category and explicit source-object root proof. Downstream report, diagram, implementation, and readiness impacts remain visible, but they are classified separately from true root causes.

Required behavior:

- A root blocker must be `BLOCKING` and `ROOT_BLOCKER`.
- Propagated report, diagram, implementation, and orchestration impacts stay visible as propagated blockers, not extra root blockers.
- Candidate/review authority gaps stay review-required unless a source engine proves a real conflict, invalid object, or missing mandatory object.
- Blocking rows without root-proof metadata become derived impacts or review-required evidence until the upstream source proves the exact root defect.
- Validation readiness may classify findings, but it must delegate root-blocker authority to `backend/src/domain/validation/root-blocker-taxonomy.ts`.

Regression guard: `node scripts/check-root-blocker-taxonomy-authority.cjs`.

### Requirement closure blocker proof

Requirement closure now requires explicit blocker proof before a requirement can be marked `BLOCKED`. A requirement may be blocked only when the closure model records at least one mandatory missing consumer, a missing required materialized source object, or an explicit engine contradiction. A `validation-blocker` materialization state without that proof stays review-required; it must not become a root blocker or inflate blocked requirement counts.

Required behavior:

- `BLOCKED` requirement lifecycle rows must include `blockedReason`.
- `validation-blocker` without blocker proof is downgraded to review-required evidence.
- Explicit no-op and inactive requirements do not create blockers.
- Scenario and report/export consumers use the same blocker-proof state and do not treat missing proof as implementation authority.
- This proof stays in backend evidence and README/tests; professional reports must show only engineer-facing readiness and review items, not developer repair language.

Regression guard: `node scripts/check-requirement-closure-blocker-proof.cjs`.

### Engine 2 IPAM authority state

Candidate IPAM is not approved authority. Engine 1 planned rows, Engine 2 candidate allocations, and Engine 2 approved allocations are separate states. Candidate allocations may support review, but only approved/implemented allocations with current hash evidence can support implementation authority. Reports must describe IPAM as planned, candidate, or approved without developer commentary or code.

### Canonical report evidence view

Reports, exports, and report-page summary cards now consume a single V1 report evidence view instead of recalculating readiness and counts independently. This view carries design-review readiness, implementation readiness, report/export readiness, diagram readiness, validation taxonomy counts, requirement closure counts, IPAM authority counts, implementation-step counts, and omitted-evidence counts.

Required behavior:

- DOCX, PDF, CSV, JSON, and the report page must use the same evidence view for blocker, review, IPAM, implementation, and omitted-evidence totals.
- Export formats must not separately calculate root blockers, IPAM authority, implementation readiness, or omitted evidence.
- Candidate IPAM and approved IPAM remain separate in the evidence view.
- Implementation candidates and execution-ready steps remain separate in the evidence view.
- Professional reports may show engineer-facing evidence summaries from this view, but must not expose developer contract names, code, repair commentary, or debug language.

Regression guard: `node scripts/check-report-evidence-view.cjs`.

### Professional report evidence boundary

The professional report now separates the engineer-facing decision document from detailed evidence appendices. Numbered report sections remain in the main report. Readiness gates, traceability evidence, omitted-evidence rollups, diagram/report evidence boundaries, and other detailed proof surfaces are preserved in appendices or machine exports instead of being mixed into the executive body.

Required behavior:

- The main report stays professional and decision-focused for a network engineer.
- Detailed evidence remains available in appendices/exports without turning the main report into a debug/proof dump.
- Developer commentary, code snippets, internal repair notes, AI/chat commentary, and debug language must not appear in report prose.
- Product proof controls such as anti-overclaim rules stay in tests/README/export evidence, not in the professional report body.
- README.md remains the only Markdown documentation file.

Regression guard: `node scripts/check-professional-report-evidence-boundary.cjs`.

### Omitted evidence decision summary

Omitted evidence remains visible, but the main export decision now uses a compact decision summary instead of forcing reviewers to interpret hundreds of sliced rows. Hidden blockers and hidden review-required rows still affect readiness, but the report surfaces the affected decision areas first: report/export, diagram, and implementation.

Required behavior:

- Omitted evidence counters remain preserved for audit and appendix/export evidence.
- The decision summary exposes total omitted rows, blocker surfaces, review surfaces, and affected report/diagram/implementation areas.
- Report/export readiness stays blocked or review-required when omitted evidence contains hidden blockers or review-required rows.
- The professional report shows an engineer-facing omitted-evidence decision summary before detailed counters.
- The frontend report page uses the canonical evidence view for omitted-evidence totals instead of recalculating them.
- README.md remains the only Markdown documentation file.

Regression guard: `node scripts/check-omitted-evidence-decision-summary.cjs`.

### Diagram graph lineage enforcement

Diagram rendering now separates graph-backed engineering evidence from visual-only presentation context. A rendered node can support implementation evidence only when it resolves to a design graph node. A rendered edge can support implementation evidence only when it resolves to a design graph relationship. Presentation-only helpers may exist for readability, but they must be explicitly labelled visual-only and non-evidence.

Required behavior:

- Every backend-rendered node carries lineage status, source refs, validation refs, readiness, truth state, and implementation-evidence status.
- Every backend-rendered edge carries lineage status, source refs, validation refs, readiness, truth state, and implementation-evidence status.
- Missing graph node or graph edge lineage blocks diagram implementation evidence instead of silently rendering clean topology.
- Visual-only presentation elements are allowed only when labelled `VISUAL_ONLY_NON_EVIDENCE` and `implementationEvidence: false`.
- The frontend may arrange or simplify visual layout, but it may not promote presentation helpers into engineering evidence.
