...
export async function createVlan(req: Request, res: Response) {
...
export async function updateVlan(req: Request, res: Response) {
  const userId = req.user?.id;
...
export async function deleteVlan(req: Request, res: Response) {
  const userId = req.user?.id;
  if (!userId) throw new ApiError(401, "Unauthorized");
