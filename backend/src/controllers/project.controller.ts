...
export async function createProjectFromTemplate(req: Request, res: Response) {
  ...
  const project = await projectService.createProjectFromTemplate(
    userId,
    planTier,
    requireParam(req, "templateKey") as "small-office" | "branch-office" | "clinic-starter",
    req.body?.name,
    req.user?.email,
  );
  res.status(201).json(project);
...
export async function getProjectSites(req: Request, res: Response) {
  const userId = req.user?.id;
  if (!userId) throw new ApiError(401, "Unauthorized");
...
export async function getProjectVlans(req: Request, res: Response) {
  const userId = req.user?.id;
  if (!userId) throw new ApiError(401, "Unauthorized");
