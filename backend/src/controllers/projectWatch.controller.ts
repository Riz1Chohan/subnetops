...
export async function listWatchers(req: Request, res: Response) {
  const userId = req.user?.id;
  if (!userId) throw new ApiError(401, "Unauthorized");
  res.json(await service.listWatchers(requireParam(req, "projectId"), userId));
}

export async function watchProject(req: Request, res: Response) {
  const userId = req.user?.id;
  if (!userId) throw new ApiError(401, "Unauthorized");
  res.status(201).json(await service.watchProject(requireParam(req, "projectId"), userId));
}

export async function unwatchProject(req: Request, res: Response) {
  const userId = req.user?.id;
  if (!userId) throw new ApiError(401, "Unauthorized");
