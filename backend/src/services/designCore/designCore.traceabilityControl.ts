...
export const V1_TRUTH_SOURCE_TYPE_POLICIES: Array<{ sourceType: DesignTruthSourceType; rule: string }> = [
...
function reviewReasonFromClosure(item: RequirementsImpactClosureItem) {
  ...
  if (item.reflectionStatus === "review-evidence") return "Captured but not currently design-driving beyond review evidence in this engine stage.";
  return undefined;
}
...
export function buildV1PlanningTraceabilityControl(input: {
  traceability: DesignTraceabilityItem[];
  ...
  return {
    ...
    sourceTypePolicy: V1_TRUTH_SOURCE_TYPE_POLICIES,
    outputLabels,
    requirementLineage,
    outputLabelCoverage: {
    ...
    notes: [
      "V1 makes truth/source/confidence labels explicit before deeper engine repairs continue.",
