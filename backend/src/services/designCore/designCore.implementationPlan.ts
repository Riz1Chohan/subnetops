...
function buildImplementationDependencyGraph(params: {
  ...
  return {
    ...
    edges,
    notes: [
      "Phase 36B promotes implementation planning from broad stage ordering to graph-driven dependency compilation.",
      "Phase 36C adds operational-safety dependencies so risky device-facing changes depend on management access, backup, and rollback readiness.",
...
function buildOperationalSafetySteps(params: {
  ...
  return params.devices.map((device, index) => {
    ...
    return buildStep({
      ...
      notes: [
        ...device.notes,
        "Phase 36C treats operational safety as a hard implementation gate, not a nice-to-have note.",
      ],
    });
  });
}
...
function buildVerificationChecks(params: {
  ...
  for (const safetyStep of params.steps.filter((step) => step.category === "operational-safety")) {
    checks.push(check({
      ...
      notes: ["Phase 36D verifies operational safety per device instead of only as a broad category."],
    }));
  }
  ...
  for (const routeIntent of params.routeIntents) {
    ...
    checks.push(check({
      ...
      notes: [
        ...
        `Administrative state: ${routeIntent.administrativeState}.`,
        "Phase 36D verifies each route intent individually instead of using one routing umbrella check.",
      ],
    }));
  }
  ...
  for (const flowRequirement of params.flowRequirements) {
    ...
    checks.push(check({
      ...
      notes: [
        ...
        "Phase 36D verifies each security flow individually instead of hiding it inside one policy check.",
      ],
    }));
  }
  ...
  checks.push(check({
    ...
    notes: ["This check is the Phase 36D guardrail against shallow implementation planning."],
  }));
  ...
  checks.push(check({
    ...
    acceptanceCriteria: [
      ...
      "Report/diagram phases can consume the verification matrix without inventing truth.",
    ],
    failureImpact: "The design may work temporarily but future engineers cannot tell what changed or what remains unsafe.",
    notes: ["This check prepares Phase 37 report/diagram truth without moving frontend authority into the browser."],
  }));
...
function buildRollbackActions(steps: ImplementationPlanStep[]) {
  ...
  if (blockedStepIds.length > 0) {
    actions.push({
      ...
      notes: ["A blocked Phase 36 step is a stop condition, not a suggestion."],
    });
  }
...
function buildImplementationFindings(params: {
  ...
  if (missingEvidenceSteps.length > 0) {
    findings.push({
      ...
      detail: `${missingEvidenceSteps.length} step(s) do not carry complete Phase 36 engineering metadata.`,
...
function buildSummary(params: {
  ...
  return {
    ...
    notes: [
      "Phase 36D expands verification from broad category checks into object-level and flow-level route, policy, NAT, DHCP, safety, rollback, and documentation checks.",
      "Phase 36C adds operational safety gates so device-facing changes require management access, backup, and rollback proof.",
