...
export const PHASE4_CIDR_ADDRESSING_TRUTH_CONTRACT_VERSION = "PHASE4_ENGINE1_CIDR_ADDRESSING_TRUTH" as const;
...
function buildEdgeCaseProofs(): Phase4CidrEdgeCaseProof[] {
  return [
...
function buildAddressingTruthRows(addressingRows: DesignCoreAddressRow[]): Phase4AddressingTruthRow[] {
...
function buildRequirementAddressingMatrix(args: {
  ...
  const phase2ByKey = new Map(args.phase2RequirementsMaterialization.fieldOutcomes.map((item) => [item.key, item]));
  const phase3ByKey = new Map((args.phase3RequirementsClosure?.closureMatrix ?? []).map((item) => [item.key, item]));

  return ADDRESSING_REQUIREMENT_POLICIES.map((policy) => {
    const phase2 = phase2ByKey.get(policy.key);
    const phase3 = phase3ByKey.get(policy.key);
    const sourceValue = phase2?.sourceValue ?? phase3?.sourceValue ?? "not captured";
    const active = Boolean(phase2?.active || phase3?.active || isActiveSourceValue(sourceValue));
    ...
    const readinessImpact: Phase4RequirementAddressingMatrixRow["readinessImpact"] = !active
      ? "NOT_APPLICABLE"
    ...
    return {
      ...
      readinessImpact,
      notes: [
        phase2 ? `Phase 2 disposition: ${phase2.expectedDisposition} / ${phase2.materializationStatus}.` : "Phase 2 policy row not captured for this key.",
        phase3 ? `Phase 3 lifecycle: ${phase3.lifecycleStatus}.` : "Phase 3 closure row not available for this key.",
        "Phase 4 does not create addressing objects from this matrix; it proves whether Engine 1 rows already reflect the requirement or exposes a review gap.",
      ],
    };
  });
}

export function buildPhase4CidrAddressingTruthControl(args: {
  siteBlocks: DesignCoreSiteBlock[];
  addressingRows: DesignCoreAddressRow[];
  ...
  const requirementAddressingMatrix = buildRequirementAddressingMatrix({
    phase2RequirementsMaterialization: args.phase2RequirementsMaterialization,
    phase3RequirementsClosure: args.phase3RequirementsClosure,
  ...
  return {
    contractVersion: PHASE4_CIDR_ADDRESSING_TRUTH_CONTRACT_VERSION,
    ...
    addressingTruthRows,
    notes: [
      "Phase 4 controls Engine 1 only: CIDR math, subnet truth, site/VLAN addressing, gateway safety, deterministic proposals, and requirement-to-addressing evidence.",
      "Engine 1 remains the mathematical planner; Phase 5 must reconcile these proposals with durable Enterprise IPAM authority.",
