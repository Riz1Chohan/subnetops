import { cidrsOverlap, parseCidr } from "../../lib/cidr.js";
import type { ParsedCidr } from "../../lib/cidr.js";
import type {
  NetworkInterface,
  NetworkLink,
...
function buildDefaultRouteIntents(project: RoutingProject, routeDomain: RouteDomain, model: RoutingNetworkObjectModel): RouteIntent[] {
  ...
  return project.sites.map((site) => ({
    ...
    notes: [
      "This is not a vendor route statement yet. It is a neutral route intent that future implementation phases can translate after edge device details are confirmed.",
    ],
  }));
}
...
function buildHubAndSpokeStaticRouteIntents(
  ...
  for (const summary of siteSummaries) {
    ...
    routeIntents.push({
      ...
      notes: [
        "Phase 34 validates this route against the backend transit-link object before treating it as usable route-table evidence.",
      ],
    });
    ...
    if (hubDestinationCidr) {
      routeIntents.push({
        ...
        notes: [
          "Return route is explicit because Phase 34 must not assume bidirectional reachability from a one-way hub route.",
        ],
      });
    }
  }
...
function buildRouteArtifacts(params: {
  ...
  const routeTables: RouteDomainRoutingTable[] = params.model.routeDomains.map((routeDomain) => {
    ...
    return {
      ...
      notes: [
        "Phase 34 route table is a backend neutral control-plane simulation. It is still not a vendor RIB/FIB dump.",
...
export function buildRoutingSegmentationModel(params: {
  ...
  return {
    summary: {
      ...
      notes: [
        "Phase 34 upgrades backend routing from basic intent to a neutral route-table and reachability simulation foundation.",
