...
function V1RequirementSummary(requirements: RequirementInputMap) {
  return [
...
function createInitialRouteDomain(project: NetworkObjectProject, siteSummaries: SiteSummarizationReview[]): RouteDomain {
  ...
  return {
    ...
    summarizationState,
    notes: [
      "V1 models a single corporate route domain from the current project data so VLAN gateways, transit links, and summaries have an explicit routing owner.",
      "Future stages can split this into VRFs or separate route domains when the product has first-class user input for routing isolation.",
    ],
  };
}
...
function addV1DefaultDenyPolicyGuardrail(
  ...
  addPolicyRule(policyRules, {
    ...
    rationale,
    notes: [
      "V1 explicit default-deny guardrail. This is design-review policy evidence, not a vendor-specific firewall command.",
...
function buildPolicyRules(securityZones: SecurityZone[], requirements: RequirementInputMap): PolicyRule[] {
  ...
  if (zoneIds.has(guestZoneId) && zoneIds.has(internalZoneId)) {
    addPolicyRule(policyRules, {
      ...
      notes: [
        "V1 keeps guest isolation as a concrete requirement-driven policy consequence, not just a VLAN label.",
  ...
  if (zoneIds.has(internalZoneId) && zoneIds.has(managementZoneId) && (requirementEnabled(requirements, "management") || hasRequirementText(requirements, "managementAccess") || hasRequirementText(requirements, "adminBoundary"))) {
    addPolicyRule(policyRules, {
      ...
      notes: [
        "V1 requires the implementation owner to narrow admin sources to jump hosts, admin workstations, or identity-aware management paths.",
  ...
  if (zoneIds.has(managementZoneId) && zoneIds.has(internalZoneId) && (hasRequirementText(requirements, "monitoringModel") || hasRequirementText(requirements, "loggingModel") || hasRequirementText(requirements, "backupPolicy") || hasRequirementText(requirements, "operationsOwnerModel") || requirementEnabled(requirements, "management"))) {
    addPolicyRule(policyRules, {
      ...
      notes: [
        "V1 turns operations selections into policy review evidence instead of report-only text.",
  ...
  if (zoneIds.has(dmzZoneId) && zoneIds.has(internalZoneId)) {
    addPolicyRule(policyRules, {
      ...
      notes: ["V1 explicit default-deny guardrail. Application exceptions belong in separate allow/review rules with exact services and owners."],
    });
  }
  ...
  if (zoneIds.has(WIDE_AREA_NETWORK_ZONE_ID) && zoneIds.has(dmzZoneId) && requirementEnabled(requirements, "remoteAccess")) {
    addPolicyRule(policyRules, {
      ...
      notes: [
        "V1 turns remote-access selection into an explicit edge-publishing consequence.",
  ...
  if (zoneIds.has(transitZoneId) && zoneIds.has(internalZoneId) && cloudOrHybridRequired(requirements)) {
    addPolicyRule(policyRules, {
      ...
      notes: [
        "V1 makes cloud provider, connectivity, routing, and traffic-boundary selections visible in policy evidence.",
  ...
  if (zoneIds.has(voiceZoneId) && (zoneIds.has(internalZoneId) || zoneIds.has(WIDE_AREA_NETWORK_ZONE_ID)) && (requirementEnabled(requirements, "voice") || requirementCountPositive(requirements, "phoneCount"))) {
    addPolicyRule(policyRules, {
      ...
      notes: [
        "V1 surfaces call-control and QoS as implementation review evidence.",
  ...
  if (zoneIds.has(iotZoneId) && zoneIds.has(internalZoneId) && (requirementEnabled(requirements, "iot") || requirementCountPositive(requirements, "iotDeviceCount") || requirementEnabled(requirements, "cameras") || requirementCountPositive(requirements, "cameraCount") || requirementEnabled(requirements, "printers") || requirementCountPositive(requirements, "printerCount"))) {
    addPolicyRule(policyRules, {
      ...
      notes: [
        "V1 turns shared-device selections into default-deny east-west policy posture.",
  ...
  addV1DefaultDenyPolicyGuardrail(policyRules, zoneIds, internalZoneId, managementZoneId, "policy-deny-general-internal-users-to-management", "Deny General Internal Users to Management Plane", "Normal corporate user networks must not broadly administer device management services; approved admin or operations sources require separate scoped review rules.");
  addV1DefaultDenyPolicyGuardrail(policyRules, zoneIds, guestZoneId, dmzZoneId, "policy-deny-guest-to-dmz", "Deny Guest to DMZ Services", "Guest networks must stay internet-only and must not reach public-service or remote-access DMZ segments.");
  addV1DefaultDenyPolicyGuardrail(policyRules, zoneIds, guestZoneId, voiceZoneId, "policy-deny-guest-to-voice", "Deny Guest Access to Voice Services", "Guest networks must not reach voice services or call-control paths by default; any exception requires explicit scoped review.");
  addV1DefaultDenyPolicyGuardrail(policyRules, zoneIds, voiceZoneId, managementZoneId, "policy-deny-voice-to-management", "Deny Voice Services to Management Plane", "Voice endpoints and call-control segments must not administer network-management services by default.");
  addV1DefaultDenyPolicyGuardrail(policyRules, zoneIds, WIDE_AREA_NETWORK_ZONE_ID, voiceZoneId, "policy-deny-wan-to-voice", "Deny Wide Area Network to Voice Services", "Untrusted WAN or internet sources must not reach voice services unless a reviewed voice edge/SBC design exists.");
  addV1DefaultDenyPolicyGuardrail(policyRules, zoneIds, WIDE_AREA_NETWORK_ZONE_ID, dmzZoneId, "policy-deny-general-wan-to-dmz", "Deny General WAN to DMZ by Default", "General inbound WAN traffic is denied by default; approved published services and remote-access edges remain separate review rules.");
  addV1DefaultDenyPolicyGuardrail(policyRules, zoneIds, guestZoneId, managementZoneId, "policy-deny-guest-to-management", "Deny Guest to Management Plane", "Guest networks must never reach device administration or management-plane services.");
  addV1DefaultDenyPolicyGuardrail(policyRules, zoneIds, guestZoneId, iotZoneId, "policy-deny-guest-to-iot", "Deny Guest to IoT and Shared Device Networks", "Guest networks should be internet-only and isolated from shared device segments.");
  addV1DefaultDenyPolicyGuardrail(policyRules, zoneIds, guestZoneId, transitZoneId, "policy-deny-guest-to-wan-transit", "Deny Guest to WAN Transit", "Guest access should not reach WAN/cloud transit segments directly; internet egress must use the reviewed WAN edge policy.");
  addV1DefaultDenyPolicyGuardrail(policyRules, zoneIds, WIDE_AREA_NETWORK_ZONE_ID, internalZoneId, "policy-deny-wan-to-corporate-internal", "Deny WAN to Corporate Internal", "Untrusted WAN or internet sources must not reach corporate internal networks by default.");
  addV1DefaultDenyPolicyGuardrail(policyRules, zoneIds, WIDE_AREA_NETWORK_ZONE_ID, managementZoneId, "policy-deny-wan-to-management", "Deny WAN to Management Plane", "External sources must not reach the management plane unless a separate, reviewed remote-admin architecture exists.");
  addV1DefaultDenyPolicyGuardrail(policyRules, zoneIds, WIDE_AREA_NETWORK_ZONE_ID, guestZoneId, "policy-deny-wan-to-guest", "Deny WAN to Guest Networks", "Guest networks are consumer-side egress segments, not inbound destinations from WAN or internet sources.");
  addV1DefaultDenyPolicyGuardrail(policyRules, zoneIds, WIDE_AREA_NETWORK_ZONE_ID, iotZoneId, "policy-deny-wan-to-iot", "Deny WAN to IoT and Shared Device Networks", "Shared device networks must not be exposed to untrusted WAN sources.");
  addV1DefaultDenyPolicyGuardrail(policyRules, zoneIds, WIDE_AREA_NETWORK_ZONE_ID, transitZoneId, "policy-deny-wan-to-wan-transit", "Deny WAN to WAN Transit Internals", "WAN/transit infrastructure should not accept broad inbound traffic from the untrusted WAN zone.");
  addV1DefaultDenyPolicyGuardrail(policyRules, zoneIds, dmzZoneId, managementZoneId, "policy-deny-dmz-to-management", "Deny DMZ to Management Plane", "Compromised DMZ workloads must not pivot into management-plane services.");
  addV1DefaultDenyPolicyGuardrail(policyRules, zoneIds, iotZoneId, managementZoneId, "policy-deny-iot-to-management", "Deny IoT to Management Plane", "Printers, cameras, and IoT devices must not reach network-device administration surfaces.");
  addV1DefaultDenyPolicyGuardrail(policyRules, zoneIds, transitZoneId, managementZoneId, "policy-deny-wan-transit-to-management", "Deny WAN Transit to Management Plane", "Transit/cloud-edge segments must not inherit management-plane access by default.");
...
function ensureRequirementDrivenZones(
  ...
  const addRequirementZone = (zoneRole: SecurityZoneRole, requirementKeys: string[], reason: string) => {
    ...
    for (const note of [
      reason,
      `V1 requirement-driven zone evidence from: ${requirementKeys.join(", ")}.`,
    ]) {
...
function sourceTypeFromTruthState(truthState: NetworkObjectTruthState): DesignTruthSourceType { if (truthState === "configured") return "USER_PROVIDED"; if (["planned","materialized","proposed"].includes(truthState)) return "REQUIREMENT_MATERIALIZED"; if (["durable","approved"].includes(truthState)) return "DURABLE_IPAM"; if (["imported","discovered"].includes(truthState)) return "IMPORTED"; if (["review-required","blocked"].includes(truthState)) return "REVIEW_REQUIRED"; return "INFERRED"; }
...
function implementationReadinessFromTruthState(truthState: NetworkObjectTruthState): V1NetworkObjectImplementationReadiness { if (["configured","durable","approved"].includes(truthState)) return "READY"; if (truthState === "blocked") return "BLOCKED"; if (["review-required","inferred","discovered","imported"].includes(truthState)) return "REVIEW_REQUIRED"; return "DRAFT_ONLY"; }
...
type V1AnnotatableObject = { id: string; name?: string; truthState: NetworkObjectTruthState; notes: string[]; sourceRequirementIds?: string[]; sourceObjectIds?: string[]; objectType?: V1NetworkObjectType; objectRole?: string; sourceType?: DesignTruthSourceType; sourceEngine?: string; confidence?: DesignTraceConfidence; proofStatus?: DesignProofStatus; implementationReadiness?: V1NetworkObjectImplementationReadiness; validationImpact?: string; frontendDisplayImpact?: string[]; reportExportImpact?: string[]; diagramImpact?: string[]; reviewReason?: string; };
function annotateObject(params: { object: V1AnnotatableObject; objectType: V1NetworkObjectType; objectRole: string; requirements: RequirementInputMap; sourceRequirementIds?: string[]; sourceObjectIds?: string[]; reviewReason?: string; }) {
  ...
  object.sourceEngine = "designCore.networkObjectModel.V1";
  ...
  object.frontendDisplayImpact = ["ProjectOverviewPage.V1NetworkObjectModel", "networkObjectModel tables"];
  object.reportExportImpact = ["V1 Network Object Model Truth", "network object lineage export rows"];
  ...
  addUnique(object.notes, `V1 object truth label: ${objectType}/${objectRole}; source=${object.sourceType}; readiness=${object.implementationReadiness}; requirements=${object.sourceRequirementIds.join(", ") || "none"}.`);
}
...
function buildObjectModelSummary(model: Omit<NetworkObjectModel, "summary">): NetworkObjectModel["summary"] {
  ...
  return {
    ...
    V1MetadataCompleteCount: allTruthStateObjects.filter((object) => Boolean(object.objectType && object.objectRole && object.sourceType && object.sourceEngine && object.confidence && object.proofStatus && object.implementationReadiness && object.validationImpact)).length,
    V1MetadataGapCount: allTruthStateObjects.filter((object) => !Boolean(object.objectType && object.objectRole && object.sourceType && object.sourceEngine && object.confidence && object.proofStatus && object.implementationReadiness && object.validationImpact)).length,
    ...
    notes: [
      "V1 object model makes devices, interfaces, links, route domains, zones, policies, NAT intent, DHCP pools, IP reservations, security flow requirements, and implementation-neutral steps explicit backend design objects.",
      ...
      "V1 adds object-level provenance/readiness metadata so every network object can be traced to requirements or an explicit review reason.",
    ],
  };
}
...
export function buildNetworkObjectModel(params: {
  ...
  routeDomain.notes.push(...V1RequirementSummary(requirements));
  ...
  wideAreaNetworkZone.notes.push(...V1RequirementSummary(requirements));
  ...
  for (const addressRow of addressingRows) {
    ...
    if (addressRow.dhcpEnabled && subnetCidr) {
      dhcpPools.push({
        ...
        notes: [
          "DHCP pool modeled from the VLAN DHCP flag. Exclusion ranges, lease duration, DNS, and relay information still require implementation-stage input.",
        ],
      });
    }
  }
  ...
  for (const transitRow of transitPlan) {
    ...
    links.push({
      ...
      notes: [
        "WAN transit modeled as a logical routed link. V1 should add the opposite endpoint when provider, firewall, or hub device details are known.",
      ],
    });
    addUnique(routeDomain.linkIds, linkId);
