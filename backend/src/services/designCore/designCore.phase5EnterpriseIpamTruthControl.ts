...
// Snapshot surface: phase5EnterpriseIpamTruth
...
}): { state: Phase5IpamReconciliationState; readinessImpact: Phase5IpamReadinessImpact; blockers: string[]; reviewReasons: string[] } {
...
function buildReconciliationRows(input: BuilderInput): Phase5EnterpriseIpamReconciliationRow[] {
...
function buildRequirementMatrix(rows: Phase5EnterpriseIpamReconciliationRow[]): Phase5EnterpriseIpamRequirementMatrixRow[] {
  return REQUIREMENT_POLICIES.map((policy) => {
    ...
    const readinessImpact: Phase5IpamReadinessImpact = !active
      ? "NOT_APPLICABLE"
      : blocking.length > 0
        ? "BLOCKING"
...
function buildConflictRows(posture: EnterpriseAllocatorPosture): Phase5EnterpriseIpamConflictRow[] {
  return posture.reviewFindings.map((finding, index) => ({
    id: `phase5-ipam-finding-${index + 1}`,
    code: finding.code,
...
export function buildPhase5EnterpriseIpamTruthControl(input: BuilderInput): Phase5EnterpriseIpamTruthControlSummary {
  ...
  const overallReadiness: Phase5IpamReadinessImpact = conflictBlockerCount > 0 || staleAllocationCount > 0
    ? "BLOCKING"
  ...
  return {
    ...
    conflictRows,
    notes: [
      "Phase 5 does not replace Engine 1. It reconciles Engine 1 planned rows against durable Engine 2 authority.",
