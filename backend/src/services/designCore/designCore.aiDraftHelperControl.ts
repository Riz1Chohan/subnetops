...
function providerMode(metadata: DraftMetadata): V1AiDraftHelperControlSummary["providerMode"] {
...
function V1Path() {
  return [
    ...
    "structured requirement/site/VLAN object with V1_AI_DRAFT_APPLIED_REVIEW_REQUIRED marker",
...
function buildGateRows(metadata: DraftMetadata, aiObjectCount: number): V1AiDraftGateRow[] {
  const hasMetadata = metadata.contract === V1_AI_DRAFT_HELPER_CONTRACT || metadata.state === "AI_DRAFT";
  const rows: V1AiDraftGateRow[] = [
    {
      ...
      required: true,
      state: "ENFORCED",
      evidence: [V1_AI_AUTHORITY, "AIPlanDraft.authority.notAuthoritative=true", "design-core V1AiDraftHelper.aiAuthority=DRAFT_ONLY_NOT_AUTHORITATIVE"],
    ...
    },
    {
      ...
      evidence: hasMetadata || aiObjectCount > 0 ? ["AI workspace stores explicit applyProjectFields/applySites/applyVlans selections", "New project flow persists V1 metadata when AI is used"] : ["No AI draft metadata detected on this project."],
      blocksAuthority: true,
    ...
    },
    {
      ...
      evidence: aiObjectCount > 0 ? ["Validation receives V1_AI_DRAFT_REVIEW_REQUIRED findings.", "AI-derived site/VLAN notes retain the V1 marker."] : ["No AI-derived site/VLAN objects detected."],
      blocksAuthority: true,
...
function buildMetadataObject(project: V1Project, metadata: DraftMetadata): V1AiDraftObjectRow | null {
  if (metadata.contract !== V1_AI_DRAFT_HELPER_CONTRACT && metadata.state !== "AI_DRAFT") return null;
  return {
    contract: V1_AI_DRAFT_HELPER_CONTRACT,
    objectId: `${project.id}:requirements:V1-ai-draft`,
    ...
    sourceRequirementIds: ["V1.aiDraft"],
    reviewRequired: true,
    materializationPath: V1Path(),
    notes: [
...
function buildObjectRows(project: V1Project, metadata: DraftMetadata): V1AiDraftObjectRow[] {
  ...
  for (const site of project.sites || []) {
    if (hasAiMarker(site.notes)) {
      rows.push({
        ...
        downstreamAuthority: "NOT_AUTHORITATIVE_UNTIL_REVIEWED",
        sourceRequirementIds: ["V1.aiDraft.site"],
        reviewRequired: true,
        materializationPath: V1Path(),
    ...
    for (const vlan of site.vlans || []) {
      ...
      rows.push({
        ...
        proofStatus: "REVIEW_REQUIRED",
        downstreamAuthority: "NOT_AUTHORITATIVE_UNTIL_REVIEWED",
        sourceRequirementIds: ["V1.aiDraft.vlan"],
        reviewRequired: true,
        materializationPath: V1Path(),
...
function buildFindings(hasMetadata: boolean, objectRows: V1AiDraftObjectRow[], gateRows: V1AiDraftGateRow[]): V1AiDraftFinding[] {
  ...
  if (missingGates.length) {
    findings.push({
      ...
      remediation: "Restore the V1 draft-only authority, selective review, structured conversion, validation, and proof gates.",
    });
  }
  ...
  if (!hasMetadata && objectRows.length === 0) {
    findings.push({
      ...
      affectedObjects: [],
      readinessImpact: "SAFE_DRAFT_ONLY",
      remediation: "No action required for V1 unless AI is used to seed this project later.",
    });
  }
...
export function buildV1AiDraftHelperControl(project: V1Project): V1AiDraftHelperControlSummary {
  const requirements = parseJsonMap(project.requirementsJson);
  const metadata = asRecord(requirements.V1AiDraft) as DraftMetadata;
  const hasMetadata = metadata.contract === V1_AI_DRAFT_HELPER_CONTRACT || metadata.state === "AI_DRAFT";
  ...
  return {
    ...
    proofBoundary: [
      ...
      "AI-created sites/VLANs must carry V1_AI_DRAFT_APPLIED_REVIEW_REQUIRED provenance when imported from the AI workspace.",
