import type {
  ...
  V1DesignGraphFinding,
  V1DesignGraphObjectCoverageRow,
  V1DesignGraphReadiness,
  V1RequirementDependencyPath,
...
const GRAPH_CONTRACT = "V1_DESIGN_GRAPH_DEPENDENCY_INTEGRITY_CONTRACT" as const;
...
function objectCoverageRows(params: { V1Objects: V1NetworkObjectLineageRow[]; graph: DesignGraph; }): V1DesignGraphObjectCoverageRow[] {
  const lookup = graphLookup(params.graph);
  return params.V1Objects.map((object) => {
...
function requirementPaths(params: { V1: V1NetworkObjectModelControlSummary; V1RequirementsClosure?: V1RequirementsClosureControlSummary; objectCoverage: V1DesignGraphObjectCoverageRow[]; graph: DesignGraph; }): V1RequirementDependencyPath[] {
  const coverageByObjectId = new Map(params.objectCoverage.map((row) => [row.objectId, row]));
  const closureByRequirementId = new Map((params.V1RequirementsClosure?.closureMatrix ?? []).map((row) => [row.requirementId, row]));
  return params.V1.requirementObjectLineage.map((row) => {
    const closure = closureByRequirementId.get(row.requirementId);
    const objectRows = row.actualObjectIds.map((objectId) => coverageByObjectId.get(objectId)).filter((item): item is V1DesignGraphObjectCoverageRow => Boolean(item));
...
function buildFindings(params: { graph: DesignGraph; networkObjectModel: NetworkObjectModel; objectCoverage: V1DesignGraphObjectCoverageRow[]; requirementPaths: V1RequirementDependencyPath[]; reportTruth?: BackendReportTruthModel; diagramTruth?: BackendDiagramTruthModel; }): V1DesignGraphFinding[] {
  ...
  for (const policy of params.networkObjectModel.policyRules.filter((rule) => !policyIdsCoveredByFlows.has(rule.id))) addFinding(findings, { severity: "WARNING", code: "V1_POLICY_WITHOUT_FLOW_COVERAGE", title: "Policy rule is not linked to a security flow requirement", detail: `${policy.name} is connected to zones, but no security-flow coverage edge proves which business/security flow it satisfies.`, affectedObjectIds: [policy.id], readinessImpact: "REVIEW_REQUIRED", remediation: "Link each policy rule to a security flow requirement or mark the policy as review-only intent." });
  for (const routeIntent of params.networkObjectModel.routingSegmentation.routeIntents.filter((route) => route.nextHopType !== "engineer-review" && !route.nextHopObjectId && route.routeKind !== "default")) addFinding(findings, { severity: "REVIEW_REQUIRED", code: "V1_ROUTE_WITHOUT_NEXT_HOP_OBJECT", title: "Route intent is missing next-hop object evidence", detail: `${routeIntent.name} has next-hop type ${routeIntent.nextHopType} but no next-hop object id.`, affectedObjectIds: [routeIntent.id], readinessImpact: "REVIEW_REQUIRED", remediation: "Attach the route intent to an interface, link, gateway, or explicit engineer-review blocker." });
  const implementationStepEdges = new Set<string>();
  for (const edge of params.graph.edges.filter((item) => item.relationship.startsWith("implementation-"))) {
    implementationStepEdges.add(edge.sourceNodeId);
    implementationStepEdges.add(edge.targetNodeId);
  }
...
export function buildV1DesignGraphControl(params: { networkObjectModel: NetworkObjectModel; V1NetworkObjectModel: V1NetworkObjectModelControlSummary; V1RequirementsClosure?: V1RequirementsClosureControlSummary; reportTruth?: BackendReportTruthModel; diagramTruth?: BackendDiagramTruthModel; }): V1DesignGraphControlSummary {
  const graph = params.networkObjectModel.designGraph;
  const objectCoverage = objectCoverageRows({ V1Objects: params.V1NetworkObjectModel.objectLineage, graph });
  const paths = requirementPaths({ V1: params.V1NetworkObjectModel, V1RequirementsClosure: params.V1RequirementsClosure, objectCoverage, graph });
