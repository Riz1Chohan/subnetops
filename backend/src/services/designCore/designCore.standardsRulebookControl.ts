...
import type {
  ...
  V1StandardsApplicabilityState,
  V1StandardsEnforcementState,
  V1StandardsFinding,
  V1StandardsRequirementActivation,
  V1StandardsRuleRow,
  V1StandardsSeverity,
  StandardsAlignmentSummary,
} from "../designCore.types.js";

export const V1_STANDARDS_ALIGNMENT_RULEBOOK_CONTRACT = "V1_STANDARDS_ALIGNMENT_RULEBOOK_CONTRACT" as const;
// V1StandardsRulebookControl snapshot field is the named DesignCoreSnapshot standards control surface.
...
function statusToEnforcementState(status: StandardsRuleStatus, required: boolean): V1StandardsEnforcementState {
...
function severityFromState(state: V1StandardsEnforcementState, required: boolean): V1StandardsSeverity {
...
function applicabilityFromStatus(status: StandardsRuleStatus): V1StandardsApplicabilityState {
...
function buildRequirementActivations(input: V1Input, ruleRows: V1StandardsRuleRow[]): V1StandardsRequirementActivation[] {
  const requirements = parseJsonMap(input.requirementsJson);
  const closureRows = input.V1RequirementsClosure.closureMatrix ?? [];
...
function findingFromRule(row: V1StandardsRuleRow): V1StandardsFinding | null {
  if (row.enforcementState === "PASS" || row.enforcementState === "NOT_APPLICABLE") return null;
  return {
    id: `V1-${row.ruleId.toLowerCase()}`,
...
export function buildV1StandardsRulebookControl(input: V1Input): V1StandardsAlignmentRulebookControlSummary {
  const requirements = parseJsonMap(input.requirementsJson);
  const ruleRows: V1StandardsRuleRow[] = input.standardsAlignment.evaluations.map((evaluation) => {
  ...
  const findings = ruleRows.map(findingFromRule).filter((item): item is V1StandardsFinding => Boolean(item));
  ...
  return {
    ...
    findings,
    notes: [
      "V1_STANDARDS_ALIGNMENT_RULEBOOK_CONTRACT turns standards into active applicability, severity, remediation, exception, and requirement-relationship evidence.",
