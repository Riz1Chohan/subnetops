import type {
  ...
  Phase16DiagramModeContractRow,
  Phase16DiagramModeKey,
  Phase16DiagramRenderCoverageRow,
  Phase16DiagramTruthControlSummary,
  Phase16DiagramTruthFinding,
  Phase16DiagramTruthReadiness,
} from "../designCore.types.js";

export const PHASE16_DIAGRAM_TRUTH_RENDERER_LAYOUT_CONTRACT = "PHASE16_DIAGRAM_TRUTH_RENDERER_LAYOUT_CONTRACT" as const;
export const PHASE16_BACKEND_ONLY_DIAGRAM_ROLE = "BACKEND_ONLY_DIAGRAM_RENDERER_NO_PRETTY_GARBAGE" as const;

type FindingSeverity = Phase16DiagramTruthFinding["severity"];

function readinessFromSeverity(severity: FindingSeverity): Phase16DiagramTruthReadiness {
  if (severity === "BLOCKING") return "BLOCKED";
  if (severity === "REVIEW_REQUIRED" || severity === "WARNING") return "REVIEW_REQUIRED";
  return "READY";
}

function rollupReadiness(values: Phase16DiagramTruthReadiness[]): Phase16DiagramTruthReadiness {
  if (values.includes("BLOCKED")) return "BLOCKED";
  if (values.includes("REVIEW_REQUIRED")) return "REVIEW_REQUIRED";
  return "READY";
}

function addFinding(findings: Phase16DiagramTruthFinding[], finding: Phase16DiagramTruthFinding) {
  if (!findings.some((existing) => existing.code === finding.code && existing.detail === finding.detail)) findings.push(finding);
}

function layersForMode(mode: Phase16DiagramModeKey): BackendDiagramRenderLayer[] {
...
function modeImpactsForNode(node: BackendDiagramRenderNode): Phase16DiagramModeKey[] {
  return (["physical", "logical", "wan-cloud", "security", "per-site", "implementation"] as Phase16DiagramModeKey[]).filter((mode) => layersForMode(mode).includes(node.layer));
}

function modeImpactsForEdge(edge: BackendDiagramRenderEdge): Phase16DiagramModeKey[] {
  const keys = new Set<Phase16DiagramModeKey>();
...
function coverageForNode(node: BackendDiagramRenderNode): Phase16DiagramRenderCoverageRow {
  return {
    rowType: "node",
    renderId: node.id,
...
function coverageForEdge(edge: BackendDiagramRenderEdge): Phase16DiagramRenderCoverageRow {
  return {
    rowType: "edge",
    renderId: edge.id,
...
function buildModeContracts(params: { diagramTruth: BackendDiagramTruthModel; networkObjectModel: NetworkObjectModel }): Phase16DiagramModeContractRow[] {
  ...
  const make = (row: Omit<Phase16DiagramModeContractRow, "contract" | "forbiddenFrontendBehavior"> & { forbiddenFrontendBehavior?: string[] }): Phase16DiagramModeContractRow => {
    ...
    return {
      contract: PHASE16_DIAGRAM_TRUTH_RENDERER_LAYOUT_CONTRACT,
      ...rest,
  ...
  return [
    ...
    make({
      ...
      notes: ["Implementation view cannot make a diagram look deployment-ready when Phase 13/14 gates are blocked or review-required."],
    }),
  ];
}

export function buildPhase16DiagramTruthControl(params: {
  ...
  const { diagramTruth, networkObjectModel, phase15ReportExportTruth } = params;
  const findings: Phase16DiagramTruthFinding[] = [];
  ...
  if (!renderModel.summary.backendAuthored) {
    addFinding(findings, {
      severity: "BLOCKING",
      code: "PHASE16_RENDER_MODEL_NOT_BACKEND_AUTHORED",
      title: "Diagram render model is not backend-authored",
      detail: "Phase 16 requires the canvas to render backend diagramTruth.renderModel only.",
      affectedRenderIds: [],
  ...
  if (nodesWithoutBackendObjectId > 0) {
    addFinding(findings, {
      severity: "BLOCKING",
      code: "PHASE16_RENDER_NODE_WITHOUT_BACKEND_OBJECT_ID",
  ...
  if (edgesWithoutRelatedObjects > 0) {
    addFinding(findings, {
      severity: "BLOCKING",
      code: "PHASE16_RENDER_EDGE_WITHOUT_BACKEND_RELATIONSHIP",
  ...
  if (renderModel.nodes.length === 0 || renderModel.edges.length === 0) {
    addFinding(findings, {
      severity: "BLOCKING",
      code: "PHASE16_EMPTY_RENDER_MODEL",
  ...
  for (const mode of modeContracts.filter((row) => row.status === "BLOCKED")) {
    addFinding(findings, {
      severity: "BLOCKING",
      code: `PHASE16_${mode.mode.toUpperCase().replace(/-/g, "_")}_MODE_BLOCKED`,
  ...
  if (phase15ReportExportTruth.overallReadiness === "BLOCKED") {
    addFinding(findings, {
      severity: "REVIEW_REQUIRED",
      code: "PHASE16_REPORT_TRUTH_BLOCKS_DIAGRAM_CONFIDENCE",
      title: "Report/export truth still has blockers",
      detail: "Phase 16 must not make diagrams look cleaner than Phase 15 report/export truth can prove.",
      affectedRenderIds: [],
      readinessImpact: "REVIEW_REQUIRED",
      remediation: "Resolve Phase 15 blockers or keep diagram/report output explicitly review-gated.",
    });
  }
  ...
  if (!findings.length) {
    findings.push({
      severity: "PASSED",
      code: "PHASE16_DIAGRAM_TRUTH_RENDERER_LAYOUT_CONTROLLED",
      ...
      remediation: "Continue to Phase 17 BOM without weakening backend-only diagram authority.",
    });
  }
  ...
  return {
    contract: PHASE16_DIAGRAM_TRUTH_RENDERER_LAYOUT_CONTRACT,
    role: PHASE16_BACKEND_ONLY_DIAGRAM_ROLE,
    overallReadiness,
    ...
    notes: [
      "Phase 16 is about diagram truth, not prettier trash.",
