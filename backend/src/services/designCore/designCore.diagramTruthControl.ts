...
<<<<<<< HEAD
  if (renderNodesMissingGraphLineage.length || renderEdgesMissingGraphLineage.length) {
    addFinding(findings, {
      severity: "BLOCKING",
      code: "V1_DIAGRAM_GRAPH_LINEAGE_MISSING",
      title: "Diagram render elements lack design graph lineage",
      detail: `${renderNodesMissingGraphLineage.length} node(s) and ${renderEdgesMissingGraphLineage.length} edge(s) cannot be resolved to design graph lineage. Visual objects without graph proof cannot be implementation evidence.`,
      affectedRenderIds: [...renderNodesMissingGraphLineage.map((node) => node.id), ...renderEdgesMissingGraphLineage.map((edge) => edge.id)],
      readinessImpact: "BLOCKED",
      remediation: "Materialize the missing design graph node/edge or mark the element visual-only and non-evidence before rendering/export.",
    });
  }

  if (visualOnlyImplementationEvidence.length) {
    addFinding(findings, {
      severity: "BLOCKING",
      code: "V1_VISUAL_ONLY_IMPLEMENTATION_EVIDENCE_LEAK",
      title: "Visual-only diagram elements were treated as implementation evidence",
      detail: `${visualOnlyImplementationEvidence.length} visual-only element(s) attempted to carry implementation evidence.`,
      affectedRenderIds: visualOnlyImplementationEvidence.map((item) => item.id),
      readinessImpact: "BLOCKED",
      remediation: "Keep visual-only presentation elements labelled non-evidence; only graph-backed objects may support implementation claims.",
    });
  }
