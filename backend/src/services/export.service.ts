...
type ValidationItem = {
  ...
  deEscalationReason?: string | null;
  remediation?: string | null;
  affectedRequirementsJson?: string | null;
  affectedObjectsJson?: string | null;
...
function isRootBlockerValidationItem(item: ValidationItem) {
  return item.readinessCategory === "BLOCKING" && item.findingClass === "ROOT_BLOCKER";
}

function isPropagatedBlockerValidationItem(item: ValidationItem) {
  return item.findingClass === "PROPAGATED_BLOCKER";
}

function validationBucket(item: ValidationItem) {
  // Export/report grouping is display-only. It must not promote findings from
  // text, title, regex, or severity into root blockers. Root-blocker authority
  // comes only from the persisted validation ledger fields.
  if (isRootBlockerValidationItem(item)) {
    return {
      severity: "ERROR",
      category: "Root blocker",
      action: asString(item.remediation, "Fix the generated source object or engine lineage before treating the plan as clean."),
    };
  }

  if (isPropagatedBlockerValidationItem(item)) {
    return {
      severity: "REVIEW",
      category: "Propagated blocker",
      action: "Fix the upstream blocker; do not count this downstream impact as a separate root blocker.",
    };
  }

  if (!hasValidationLedgerTaxonomy(item)) {
    return {
      severity: "REVIEW",
      category: "Validation ledger taxonomy missing",
      action: "Regenerate validation so the export can consume authoritative readiness category and finding class fields.",
    };
  }
...
export function buildExportContext(project: Awaited<ReturnType<typeof getProjectExportData>>) {
  ...
  const errors = validations.filter(isRootBlockerValidationItem);
  const warnings = validations.filter((item) => item.severity !== "INFO" && !isRootBlockerValidationItem(item));
...
>>>>>>> 620cdbb100bc3a54420d680ba278e3b8cad06da8
  ...
  const platformSection: ReportSection = {
    ...
    paragraphs: [
      ...
      "Platform/BOM readiness must be read through the saved platform profile state. If no saved platform profile exists, the package is not procurement-ready; role-based assumptions remain useful for engineering review but cannot be treated as a quote, final SKU list, or purchasing approval.",
...
>>>>>>> 620cdbb100bc3a54420d680ba278e3b8cad06da8
  ...
  rows.push({
    Section: "V1 Report Evidence View",
    Scope: reportEvidenceView.readiness.reportExport,
    Name: "Canonical export evidence",
    Key: reportEvidenceView.contract,
    Value: `design ${reportEvidenceView.readiness.designReview}; implementation ${reportEvidenceView.readiness.implementation}; diagram ${reportEvidenceView.readiness.diagram}`,
    Notes: `Root blockers ${reportEvidenceView.validation.rootBlockerCount}; propagated blockers ${reportEvidenceView.validation.propagatedBlockerCount}; review items ${reportEvidenceView.validation.reviewItemCount}; candidate IPAM ${reportEvidenceView.ipam.candidateAllocations}; approved IPAM ${reportEvidenceView.ipam.approvedAllocations}; executable steps ${reportEvidenceView.implementation.executableSteps}; omitted rows ${reportEvidenceView.omittedEvidence.omittedRows}`,
  });
  const exportConsistencyProof = designCore?.V1ReportExportTruth?.exportConsistencyProof ?? buildRuntimeExportConsistencyProof({ designCore, evidenceView: reportEvidenceView });
  rows.push({
    Section: "V1 Export Consistency Proof",
    Scope: exportConsistencyProof.killSwitchPassed ? "passed" : "review",
    Name: "Runtime export consistency",
    Key: exportConsistencyProof.contract,
    Value: `Canonical root blockers ${exportConsistencyProof.canonicalRootBlockerCount}; counts agree ${exportConsistencyProof.countsAgree ? "yes" : "no"}`,
    Notes: `CSV root blockers ${exportConsistencyProof.surfaces.find((surface: any) => surface.surface === "CSV")?.rootBlockerCount ?? reportEvidenceView.validation.rootBlockerCount}; PDF root blockers ${exportConsistencyProof.surfaces.find((surface: any) => surface.surface === "PDF")?.rootBlockerCount ?? reportEvidenceView.validation.rootBlockerCount}; DOCX root blockers ${exportConsistencyProof.surfaces.find((surface: any) => surface.surface === "DOCX")?.rootBlockerCount ?? reportEvidenceView.validation.rootBlockerCount}; frontend root blockers ${exportConsistencyProof.surfaces.find((surface: any) => surface.surface === "FRONTEND")?.rootBlockerCount ?? reportEvidenceView.validation.rootBlockerCount}; candidate IPAM approved-label violations ${exportConsistencyProof.candidateIpamRowsLabelledApprovedAuthority}; blocked requirements without proof ${exportConsistencyProof.blockedRequirementRowsWithoutProof}`,
  });
...
<<<<<<< HEAD
      if (designCore.V1ReportExportTruth.omittedEvidenceDecisionSummary) {
        ...
        rows.push({
          Section: "V1 Report Omitted Evidence Decision",
          Scope: "Project",
          Name: "Omitted evidence decision summary",
          Key: decision.decisionImpact,
          Value: `${decision.totalOmittedRows} omitted row(s); ${decision.blockingSurfaces?.length ?? 0} blocker surface(s); ${decision.reviewSurfaces?.length ?? 0} review surface(s)`,
...
export async function getJsonExport(projectId: string) {
  const project = await getProjectExportData(projectId);
  const designCore = await getDesignCoreSnapshotForExport(projectId);
  const evidenceView = designCore?.V1ReportExportTruth?.evidenceView ?? buildReportEvidenceView({ designCore });
  const exportConsistencyProof = designCore?.V1ReportExportTruth?.exportConsistencyProof ?? buildRuntimeExportConsistencyProof({ designCore, evidenceView });
  return {
    contract: "JSON_EXPORT_CONSUMES_V1_REPORT_EVIDENCE_VIEW",
    project: { id: project.id, name: project.name, environmentType: project.environmentType ?? null },
    evidenceView,
