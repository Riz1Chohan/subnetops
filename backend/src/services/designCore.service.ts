...
import { buildV1PlanningTraceabilityControl } from "./designCore/designCore.traceabilityControl.js";
import { buildV1RequirementsClosureControl } from "./designCore/designCore.requirementsClosureControl.js";
import { buildV1CidrAddressingTruthControl } from "./designCore/designCore.cidrAddressingTruthControl.js";
import { buildV1EnterpriseIpamTruthControl } from "./designCore/designCore.enterpriseIpamTruthControl.js";
import { buildV1DesignCoreOrchestratorControl } from "./designCore/designCore.designCoreOrchestratorControl.js";
import { buildV1StandardsRulebookControl } from "./designCore/designCore.standardsRulebookControl.js";
import { buildV1ValidationReadinessControl } from "./designCore/designCore.validationReadinessControl.js";
import { buildV1NetworkObjectModelControl } from "./designCore/designCore.networkObjectModelControl.js";
import { buildV1DesignGraphControl } from "./designCore/designCore.designGraphControl.js";
import { buildV1RoutingSegmentationControl } from "./designCore/designCore.routingSegmentationControl.js";
import { buildV1SecurityPolicyFlowControl } from "./designCore/designCore.securityPolicyFlowControl.js";
import { buildV1ImplementationPlanningControl } from "./designCore/designCore.implementationPlanningControl.js";
import { buildV1ImplementationTemplatesControl } from "./designCore/designCore.implementationTemplatesControl.js";
import { buildV1ReportExportTruthControl } from "./designCore/designCore.reportExportTruthControl.js";
import { buildV1DiagramTruthControl } from "./designCore/designCore.diagramTruthControl.js";
...
export function buildDesignCoreSnapshot(project: ProjectWithDesignData): DesignCoreSnapshot | null {
  ...
  const V1RequirementsMaterialization = buildRequirementMaterializationPolicySummary(
  ...
  const V1TraceabilityControl = buildV1PlanningTraceabilityControl({
    traceability,
  ...
  const V1RequirementsClosure = buildV1RequirementsClosureControl({
  ...
  // V1_ENGINE1_CIDR_ADDRESSING_TRUTH: Engine 1 CIDR/addressing proof surface.
  const V1CidrAddressingTruth = buildV1CidrAddressingTruthControl({
    siteBlocks,
    addressingRows,
    proposedRows: proposals,
    issues,
  ...
  // V1_DESIGN_CORE_ORCHESTRATOR_CONTRACT: design-core boundary/coordination proof surface.
  const V1DesignCoreOrchestrator = buildV1DesignCoreOrchestratorControl({
  ...
  // V1_STANDARDS_ALIGNMENT_RULEBOOK_CONTRACT: active standards applicability/severity/remediation proof surface.
  const V1StandardsRulebookControl = buildV1StandardsRulebookControl({
    requirementsJson: project.requirementsJson,
    siteCount: project.sites.length,
    standardsAlignment,
    V1RequirementsClosure,
  ...
  // V1_VALIDATION_READINESS_AUTHORITY_CONTRACT: strict validation/readiness authority aggregation surface.
  const V1ValidationReadiness = buildV1ValidationReadinessControl({
    projectId: project.id,
    V1RequirementsClosure,
    V1CidrAddressingTruth,
    V1EnterpriseIpamTruth,
  ...
  // V1_NETWORK_OBJECT_MODEL_TRUTH_CONTRACT: truth-labelled device/interface/link/zone/policy/NAT/DHCP/reservation object authority.
  const V1NetworkObjectModel = buildV1NetworkObjectModelControl({ networkObjectModel, V1RequirementsClosure });

  // V1_DESIGN_GRAPH_DEPENDENCY_INTEGRITY_CONTRACT: requirement-to-object-to-consumer graph dependency proof surface.
  const V1DesignGraph = buildV1DesignGraphControl({ networkObjectModel, V1NetworkObjectModel, V1RequirementsClosure, reportTruth, diagramTruth });

  // V1_ROUTING_SEGMENTATION_PROTOCOL_AWARE_PLANNING_CONTRACT: routing/segmentation intent, review, blocker, and simulation-unavailable proof surface.
  const V1RoutingSegmentation = buildV1RoutingSegmentationControl({ project, networkObjectModel, V1DesignGraph });
  const V1SecurityPolicyFlow = buildV1SecurityPolicyFlowControl({ project, networkObjectModel, V1DesignGraph, V1RoutingSegmentation });

  // V1_IMPLEMENTATION_PLANNING_CONTRACT: verified source-object gated implementation steps, evidence, rollback, and readiness.
  const V1ImplementationPlanning = buildV1ImplementationPlanningControl({ networkObjectModel, V1NetworkObjectModel, V1DesignGraph, V1RoutingSegmentation, V1SecurityPolicyFlow });

  // V1_VENDOR_NEUTRAL_IMPLEMENTATION_TEMPLATES_CONTRACT: template variables/evidence/rollback without vendor commands.
  const V1ImplementationTemplates = buildV1ImplementationTemplatesControl({ vendorNeutralImplementationTemplates, V1ImplementationPlanning });

  // V1_REPORT_EXPORT_TRUTH_CONTRACT: report/export deliverable truth gate with requirement traceability matrix.
  const V1ReportExportTruth = buildV1ReportExportTruthControl({
    reportTruth,
    diagramTruth,
    V1RequirementsClosure,
    V1CidrAddressingTruth,
  ...
  // V1_DIAGRAM_TRUTH_RENDERER_LAYOUT_CONTRACT: backend-only diagram render authority; no pretty garbage.
  const V1DiagramTruth = buildV1DiagramTruthControl({ diagramTruth, networkObjectModel, V1ReportExportTruth });

  // V1_PLATFORM_BOM_FOUNDATION_CONTRACT: backend-controlled advisory BOM foundation; no fake SKUs.
  const V1PlatformBomFoundation = buildV1PlatformBomFoundationControl({ project, networkObjectModel, V1DiagramTruth });

  // V1_DISCOVERY_CURRENT_STATE_CONTRACT: backend discovery/current-state control; manual/imported evidence only, no live discovery claims.
  const V1DiscoveryCurrentState = buildV1DiscoveryCurrentStateControl({
    project,
    networkObjectModel,
    currentStateBoundary,
    brownfieldReadiness,
