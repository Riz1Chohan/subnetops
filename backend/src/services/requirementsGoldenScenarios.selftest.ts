...
function withDefaults(requirements: RequirementMap): RequirementMap {
  return {
    ...
    implementationTimeline: "staged implementation",
    ...
    customRequirementsNotes: "Generated by V1 golden scenario selftest.",
    ...requirements,
  };
}
  ...
  return {
    ...
    toDesignProject() {
      return {
        ...
        organizationId: null,
        name: project.name,
        description: "V1 golden scenario runtime selftest project.",
...
const scenarios: GoldenScenario[] = [
  ...
  },
  {
    ...
    requirements: withDefaults({
      ...
      projectStage: "brownfield upgrade",
      siteCount: 2,
      usersPerSite: 40,
      environmentType: "hybrid",
      cloudConnected: true,
      ...
      rolloutModel: "staged migration by site",
...
async function runScenario(scenario: GoldenScenario) {
  ...
  const summary = await materializeRequirementsForProject(db.tx as any, project.id, "V1-golden-scenario-selftest");
  ...
  assert(snapshot.V1RequirementsClosure.contractVersion === "V1_REQUIREMENTS_IMPACT_CLOSURE_SCENARIO_PROOF", `${scenario.id}: V1 closure contract missing`);
  assert(snapshot.V1RequirementsClosure.activeRequirementCount > 0, `${scenario.id}: V1 closure found no active requirements`);
  assert(snapshot.V1RequirementsClosure.closureMatrix.length >= snapshot.V1RequirementsMaterialization.totalPolicyCount, `${scenario.id}: V1 closure matrix did not cover V1 policy rows`);
  assert(snapshot.V1RequirementsClosure.goldenScenarioClosures.some((item) => item.relevant), `${scenario.id}: V1 did not mark any golden scenario relevant`);
  ...
  return {
    ...
    V1Closure: `${snapshot.V1RequirementsClosure.fullPropagatedCount}/${snapshot.V1RequirementsClosure.activeRequirementCount}`,
    V1Blocked: snapshot.V1RequirementsClosure.blockedCount,
  };
}
...
async function main() {
  ...
  console.log("V1 requirements golden scenario runtime selftest passed.");
