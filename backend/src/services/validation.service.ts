// V1_DISCOVERY_CURRENT_STATE_CONTRACT validation wiring
// V1_PLATFORM_BOM_FOUNDATION_CONTRACT validation wiring
// V1_DIAGRAM_TRUTH_RENDERER_LAYOUT_CONTRACT: validation consumes diagram truth findings.
...
function severityForV1ValidationFinding(finding: { category?: string; findingClass?: string }): ValidationSeverity {
  return severityForValidationLedger({
    readinessCategory: finding.category as ValidationLedgerReadinessCategory | undefined,
    findingClass: finding.findingClass as ValidationLedgerFindingClass | undefined,
  });
}
...
const validationGuidanceByRuleCode: Record<string, Partial<ValidationNarrative>> = {
  ...
  IPAM_DURABLE_AUTHORITY_BLOCKED: {
    impact: "Engine 1 planned addressing is not safe to implement while Engine 2 Engine 2 IPAM reports a blocker such as brownfield overlap, stale approval, reserved pool breach, DHCP conflict, or reservation conflict.",
    recommendation: "Resolve the Engine 2 IPAM blocker, update the candidate/approved allocation, approval, and ledger state, and rerun validation before implementation handoff.",
  },
  IPAM_DURABLE_AUTHORITY_REVIEW_REQUIRED: {
    impact: "The subnet may be mathematically valid, but it is not Engine 2 IPAM authority yet. Treating it as ready would create split-brain addressing risk.",
...
<<<<<<< HEAD
  ...
  const ledger = buildValidationLedgerFields({
    ...item,
    message,
    affectedObjects: item.affectedObjects ?? (item.entityId ? [item.entityId] : []),
  });
  return {
    ...item,
    ...ledger,
...
export async function runValidation(projectId: string) {
  ...
  if (designSnapshot?.V1EnterpriseIpamTruth) {
    const V1 = designSnapshot.V1EnterpriseIpamTruth;
    for (const row of V1.reconciliationRows) {
    ...
    for (const item of V1.requirementIpamMatrix.filter((field) => field.active && field.readinessImpact !== "PASSED" && field.readinessImpact !== "NOT_APPLICABLE")) {
      results.push(makeItem({
        projectId,
  ...
  if (designSnapshot?.V1DesignCoreOrchestrator) {
    const V1 = designSnapshot.V1DesignCoreOrchestrator;
    for (const finding of V1.boundaryFindings) {
      results.push(makeItem({
        projectId,
  ...
  if (designSnapshot?.V1StandardsRulebookControl) {
    const V1 = designSnapshot.V1StandardsRulebookControl;
...
<<<<<<< HEAD
        ...
        sourceSnapshotPath: finding.sourceSnapshotPath,
        rootCauseKey: finding.rootCauseKey ?? finding.ruleCode,
        rootCauseTitle: finding.rootCauseTitle ?? finding.title,
        deEscalationReason: finding.deEscalationReason,
...
>>>>>>> 620cdbb100bc3a54420d680ba278e3b8cad06da8
  ...
  if (designSnapshot?.V1NetworkObjectModel) {
    for (const finding of designSnapshot.V1NetworkObjectModel.findings.filter((item) => item.severity !== "PASSED" && item.severity !== "INFO")) {
      results.push(makeItem({ projectId, severity: finding.severity === "BLOCKING" ? "ERROR" : "WARNING", ruleCode: finding.severity === "BLOCKING" ? "V1_NETWORK_OBJECT_MODEL_BLOCKING" : "V1_NETWORK_OBJECT_MODEL_REVIEW_REQUIRED", title: finding.title, message: `${finding.detail} Remediation: ${finding.remediation}`, entityType: "PROJECT", entityId: projectId }));
    }
  }

  if (designSnapshot?.V1DesignGraph) {
    for (const finding of designSnapshot.V1DesignGraph.findings.filter((item) => item.severity !== "PASSED" && item.severity !== "INFO")) {
      results.push(makeItem({ projectId, severity: finding.severity === "BLOCKING" ? "ERROR" : "WARNING", ruleCode: finding.severity === "BLOCKING" ? "V1_DESIGN_GRAPH_BLOCKING" : "V1_DESIGN_GRAPH_REVIEW_REQUIRED", title: finding.title, message: `${finding.detail} Remediation: ${finding.remediation}`, entityType: "PROJECT", entityId: projectId }));
    }
  }

  if (designSnapshot?.V1RoutingSegmentation) {
    for (const finding of designSnapshot.V1RoutingSegmentation.findings.filter((item) => item.severity !== "PASSED" && item.severity !== "INFO")) {
      results.push(makeItem({ projectId, severity: finding.severity === "BLOCKING" ? "ERROR" : "WARNING", ruleCode: finding.severity === "BLOCKING" ? "V1_ROUTING_SEGMENTATION_BLOCKING" : "V1_ROUTING_SEGMENTATION_REVIEW_REQUIRED", title: finding.title, message: `${finding.detail} Remediation: ${finding.remediation}`, entityType: "PROJECT", entityId: projectId }));
    }
  }


  if (designSnapshot?.V1SecurityPolicyFlow) {
    for (const finding of designSnapshot.V1SecurityPolicyFlow.findings.filter((item) => item.severity !== "PASSED" && item.severity !== "INFO")) {
      results.push(makeItem({ projectId, severity: finding.severity === "BLOCKING" ? "ERROR" : "WARNING", ruleCode: finding.severity === "BLOCKING" ? "V1_SECURITY_POLICY_FLOW_BLOCKING" : "V1_SECURITY_POLICY_FLOW_REVIEW_REQUIRED", title: finding.title, message: `${finding.detail} Remediation: ${finding.remediation}`, entityType: "PROJECT", entityId: projectId }));
    }
  }
...
export async function getValidationResults(projectId: string) {
  // V1: validation reads must reconcile with the same read-repaired
  // materialized evidence used by design-core/report. Returning old persisted
  // ValidationResult rows after V1 can falsely claim 0 sites/0 VLANs
