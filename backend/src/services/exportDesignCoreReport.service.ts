...
export function applyBackendDesignCoreToReport(report: ProfessionalReport, designCore: any, options?: { reportMode?: ProfessionalReportMode }) {
  ...
  if (addressingSection) {
    ...
    if (includeTechnicalEvidence && phase9NetworkObjectModel) {
      report.sections.push({ title: "Phase 9 Network Object Model Truth", paragraphs: ["The network object model is truth-labelled so devices, interfaces, links, zones, policies, NAT intent, DHCP pools, and reservations cannot masquerade as approved topology without source/readiness evidence."], tables: [{ title: "Phase 9 Object Model Summary", headers: ["Gate", "Status", "Evidence"], rows: [["Contract", asString(phase9NetworkObjectModel.contract, "missing"), asString(phase9NetworkObjectModel.role, "object-model truth")], ["Overall readiness", asString(phase9NetworkObjectModel.overallReadiness, "review"), `${phase9NetworkObjectModel.objectCount ?? 0} object(s); ${phase9NetworkObjectModel.metadataGapObjectCount ?? 0} metadata gap(s); ${phase9NetworkObjectModel.requirementLineageGapCount ?? 0} requirement lineage gap(s).`]] }, { title: "Phase 9 Object Lineage Rows", headers: ["Object", "Type", "Truth", "Readiness", "Requirements"], rows: compactRows(asArray(phase9NetworkObjectModel.objectLineage).slice(0, 36).map((row: any) => [asString(row.displayName), asString(row.objectType), asString(row.truthState), asString(row.implementationReadiness), joinText(asArray(row.sourceRequirementIds).slice(0, 4), "none")]), [["No Phase 9 object lineage rows", "—", "—", "—", "—"]]) }, { title: "Phase 9 Requirement-to-Object Lineage", headers: ["Requirement", "Lifecycle", "Readiness", "Actual", "Missing"], rows: compactRows(asArray(phase9NetworkObjectModel.requirementObjectLineage).slice(0, 30).map((row: any) => [asString(row.sourceKey), asString(row.lifecycleStatus), asString(row.readinessImpact), joinText(asArray(row.actualObjectTypes), "none"), joinText(asArray(row.missingObjectTypes), "none")]), [["No Phase 9 requirement lineage rows", "—", "—", "—", "—"]]) }, { title: "Phase 9 Object Findings", headers: ["Severity", "Code", "Readiness", "Finding", "Remediation"], rows: compactRows(asArray(phase9NetworkObjectModel.findings).filter((finding: any) => asString(finding.severity) !== "PASSED").slice(0, 30).map((finding: any) => [asString(finding.severity), asString(finding.code), asString(finding.readinessImpact), asString(finding.title), asString(finding.remediation)]), [["PASSED", "NETWORK_OBJECT_MODEL_TRUTH_LABELS_COMPLETE", "READY", "No Phase 9 findings", "Continue engineer review."]]) }] });
    }

    if (includeTechnicalEvidence && phase10DesignGraph) {
      report.sections.push({ title: "Phase 10 Design Graph Dependency Integrity", paragraphs: ["The design graph is the dependency integrity layer. It proves requirement → object → graph relationship → validation/frontend/report/diagram consumer paths, and blocks diagram-only or orphaned topology authority."], tables: [{ title: "Phase 10 Graph Summary", headers: ["Gate", "Status", "Evidence"], rows: [["Contract", asString(phase10DesignGraph.contract, "missing"), asString(phase10DesignGraph.role, "dependency graph")], ["Overall readiness", asString(phase10DesignGraph.overallReadiness, "review"), `${phase10DesignGraph.graphNodeCount ?? 0} node(s); ${phase10DesignGraph.graphEdgeCount ?? 0} edge(s); ${phase10DesignGraph.objectCoverageGapCount ?? 0} object coverage gap(s).`], ["Requirement paths", `${phase10DesignGraph.requirementPathReadyCount ?? 0} ready`, `${phase10DesignGraph.requirementPathReviewCount ?? 0} review; ${phase10DesignGraph.requirementPathBlockedCount ?? 0} blocked.`]] }, { title: "Phase 10 Requirement Dependency Paths", headers: ["Requirement", "Lifecycle", "Readiness", "Graph nodes", "Missing"], rows: compactRows(asArray(phase10DesignGraph.requirementDependencyPaths).slice(0, 30).map((row: any) => [asString(row.sourceKey), asString(row.lifecycleStatus), asString(row.readinessImpact), joinText(asArray(row.actualGraphNodeIds).slice(0, 5), "none"), joinText([...asArray(row.missingGraphNodeIds), ...asArray(row.missingConsumerSurfaces)], "none")]), [["No Phase 10 requirement paths", "—", "—", "—", "—"]]) }, { title: "Phase 10 Object Graph Coverage", headers: ["Object", "Type", "State", "Relationships", "Consumers"], rows: compactRows(asArray(phase10DesignGraph.objectCoverage).slice(0, 36).map((row: any) => [asString(row.displayName), asString(row.objectType), asString(row.dependencyState), joinText(asArray(row.relationshipTypes).slice(0, 5), "none"), joinText(asArray(row.consumerSurfaces).slice(0, 5), "none")]), [["No Phase 10 object coverage rows", "—", "—", "—", "—"]]) }, { title: "Phase 10 Graph Findings", headers: ["Severity", "Code", "Readiness", "Finding", "Remediation"], rows: compactRows(asArray(phase10DesignGraph.findings).filter((finding: any) => asString(finding.severity) !== "PASSED").slice(0, 30).map((finding: any) => [asString(finding.severity), asString(finding.code), asString(finding.readinessImpact), asString(finding.title), asString(finding.remediation)]), [["PASSED", "PHASE10_DESIGN_GRAPH_DEPENDENCIES_COMPLETE", "READY", "No Phase 10 findings", "Continue engineer review."]]) }] });
    }

    if (includeTechnicalEvidence && phase11RoutingSegmentation) {
