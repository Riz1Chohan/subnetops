...
<<<<<<< HEAD
        ...
        rows: [[
          reportOmittedEvidenceDecisionSummary.decisionImpact,
          String(reportOmittedEvidenceDecisionSummary.totalOmittedRows),
          String(reportOmittedEvidenceDecisionSummary.blockingSurfaces.length),
          String(reportOmittedEvidenceDecisionSummary.reviewSurfaces.length),
          reportOmittedEvidenceDecisionSummary.reportAffected ? "affected" : "not affected",
          reportOmittedEvidenceDecisionSummary.diagramAffected ? "affected" : "not affected",
          reportOmittedEvidenceDecisionSummary.implementationAffected ? "affected" : "not affected",
        ]],
      },
      {
        title: "Omitted Evidence Affected Surfaces",
        headers: ["Impact", "Surfaces"],
        rows: [
          ["Blocking", joinText(reportOmittedEvidenceDecisionSummary.blockingSurfaces.slice(0, 8), "none")],
          ["Review", joinText(reportOmittedEvidenceDecisionSummary.reviewSurfaces.slice(0, 8), "none")],
        ],
      },
      {
=======
...
<<<<<<< HEAD
    ...
    if (includeTechnicalEvidence && V1ImplementationTemplates) {
      report.sections.push({ title: "V1 Vendor-Neutral Implementation Templates", paragraphs: ["V1 compiles vendor-neutral templates from the backend implementation plan and V1 gates. It exposes variables, source objects, source requirements, missing-data blockers, neutral actions, evidence requirements, rollback requirements, and command-generation boundaries without emitting vendor CLI/cloud commands."], tables: [{ title: "V1 Template Summary", headers: ["Gate", "Status", "Evidence"], rows: [["Contract", asString(V1ImplementationTemplates.contract, "missing"), asString(V1ImplementationTemplates.role, "vendor-neutral template control")], ["Overall readiness", asString(V1ImplementationTemplates.overallReadiness, "review"), `${V1ImplementationTemplates.templateCount ?? 0} template gate(s); ${V1ImplementationTemplates.blockedTemplateCount ?? 0} blocked; ${V1ImplementationTemplates.reviewTemplateCount ?? 0} review.`], ["Command generation", V1ImplementationTemplates.commandGenerationAllowed ? "enabled" : "disabled", `${V1ImplementationTemplates.vendorSpecificCommandCount ?? 0} vendor command(s); ${V1ImplementationTemplates.commandLeakCount ?? 0} command leak(s).`]] }, { title: "V1 Template Gates", headers: ["Template", "Domain", "Readiness", "Sources", "Requirements", "Evidence", "Rollback"], rows: compactRows(asArray(V1ImplementationTemplates.templateGates).slice(0, 40).map((row: any) => [asString(row.title), asString(row.domain), asString(row.readinessImpact), joinText(asArray(row.sourceObjectIds).slice(0, 4), "none"), joinText(asArray(row.sourceRequirementIds).slice(0, 4), "none"), joinText(asArray(row.evidenceRequired).slice(0, 3), "missing evidence"), asString(row.rollbackRequirement, "missing rollback")]), [["No V1 template gates", "—", "—", "—", "—", "—", "—"]]) }, { title: "Implementation Findings", headers: ["Severity", "Code", "Readiness", "Finding", "Remediation"], rows: compactRows(asArray(V1ImplementationTemplates.findings).filter((finding: any) => asString(finding.severity) !== "PASSED").slice(0, 30).map((finding: any) => [asString(finding.severity), asString(finding.code), asString(finding.readinessImpact), asString(finding.title), asString(finding.remediation)]), [["PASSED", "V1_VENDOR_NEUTRAL_TEMPLATES_CONTROLLED", "READY", "No V1 findings", "Continue to report/export truth without vendor command generation."]]) }] });
    }


    if (includeTechnicalEvidence) {
      report.sections.push({
        title: "V1 Canonical Report Evidence View",
        paragraphs: ["The report, exports, and report-page summaries consume one authoritative evidence view for readiness and counts. This prevents separate formats from calculating different blocker, IPAM, implementation, or omitted-evidence totals."],
        tables: [
          {
            title: "Canonical Evidence View",
            headers: ["Area", "Status", "Evidence"],
            rows: [
              ["Design review", reportEvidenceView.readiness.designReview, `root blockers ${reportEvidenceView.validation.rootBlockerCount}; review items ${reportEvidenceView.validation.reviewItemCount}; warnings ${reportEvidenceView.validation.warningCount}`],
              ["Implementation", reportEvidenceView.readiness.implementation, `execution-ready ${reportEvidenceView.implementation.executableSteps}; planning candidates ${reportEvidenceView.implementation.planningCandidateSteps}; review ${reportEvidenceView.implementation.reviewSteps}; blocked ${reportEvidenceView.implementation.blockedSteps}`],
              ["IPAM", reportEvidenceView.ipam.approvedAllocations > 0 ? "approved evidence present" : "review-required", `planned ${reportEvidenceView.ipam.plannedRows}; candidates ${reportEvidenceView.ipam.candidateAllocations}; approved ${reportEvidenceView.ipam.approvedAllocations}; stale ${reportEvidenceView.ipam.staleAllocations}; conflicts ${reportEvidenceView.ipam.conflictBlockers}`],
              ["Report/export", reportEvidenceView.readiness.reportExport, `hidden blocker surfaces ${reportEvidenceView.omittedEvidence.hiddenBlockerSurfaces}; hidden review surfaces ${reportEvidenceView.omittedEvidence.hiddenReviewSurfaces}; omitted rows ${reportEvidenceView.omittedEvidence.omittedRows}`],
              ["Diagram", reportEvidenceView.readiness.diagram, "Diagram readiness is read from the same evidence view used by exports."],
            ],
          },
        ],
      });
