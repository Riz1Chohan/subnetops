...
export function buildOmittedEvidenceDecisionSummary(summaries: OmittedEvidenceSummary[]): OmittedEvidenceDecisionSummary {
  const surfaces = summaries.filter((summary) => summary.omittedCount > 0);
  const blockingSurfaces = surfaces
    .filter((summary) => summary.omittedHasBlockers || summary.readinessImpact === "BLOCKING")
    .map((summary) => `${summary.collection} (${summary.surface})`);
  const reviewSurfaces = surfaces
    .filter((summary) => !blockingSurfaces.includes(`${summary.collection} (${summary.surface})`) && (summary.omittedHasReviewRequired || summary.readinessImpact === "REVIEW"))
    .map((summary) => `${summary.collection} (${summary.surface})`);
  const totalOmittedRows = surfaces.reduce((sum, row) => sum + row.omittedCount, 0);
  const text = surfaces.map((summary) => `${summary.collection} ${summary.surface}`).join(" ").toLowerCase();
  const implementationAffected = /implementation|template|rollback|verification/.test(text) || blockingSurfaces.some((surface) => /implementation|template|rollback|verification/i.test(surface));
  const reportAffected = /report|traceability|export|requirement/.test(text) || blockingSurfaces.some((surface) => /report|traceability|export|requirement/i.test(surface));
  const diagramAffected = /diagram|render|node|edge|topology/.test(text) || blockingSurfaces.some((surface) => /diagram|render|node|edge|topology/i.test(surface));
  const decisionImpact = blockingSurfaces.length > 0 ? "BLOCKING" : reviewSurfaces.length > 0 ? "REVIEW" : "NONE";
  return {
    totalSurfaces: summaries.length,
    totalOmittedRows,
    blockingSurfaces,
    reviewSurfaces,
    implementationAffected,
    reportAffected,
    diagramAffected,
    decisionImpact,
  };
