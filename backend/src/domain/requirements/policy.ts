...
import type { RequirementSourceType } from "./types.js";

export const REQUIREMENTS_MATERIALIZATION_POLICY_CONTRACT_VERSION =
  "V1_REQUIREMENTS_MATERIALIZATION_POLICY_CONTRACT" as const;
...
export type RequirementMaterializationConfidence =
  | "high"
  | "medium"
  | "low"
  | "advisory";
...
export type RequirementMaterializationOutcome =
  RequirementMaterializationPolicy & {
    sourceValue: string;
    captured: boolean;
    active: boolean;
...
function policyDispositionFor(
  item: RequirementImpactRegistryItem,
  ...
  if (item.category === "constraints" || item.category === "handoff")
    return "REVIEW_ITEM";
  return "ENGINE_INPUT_SIGNAL";
}

function confidenceFor(
  disposition: RequirementMaterializationDisposition,
  item: RequirementImpactRegistryItem,
): RequirementMaterializationConfidence {
...
function objectTypesFor(
  ...
  if (segments.length > 0) {
    ...
    if (
      ["WAN-TRANSIT", "CLOUD-EDGE", "REMOTE-ACCESS"].some((name) =>
  ...
  if (disposition === "VALIDATION_BLOCKER")
    return ["Validation blocker/review signal"];
  if (disposition === "REVIEW_ITEM") return ["Review item"];
  return [];
}

function engineInputsFor(
  item: RequirementImpactRegistryItem,
  disposition: RequirementMaterializationDisposition,
) {
  const inputs = ["designCore.requirementContext", "designCore.traceability"];
  if (
    disposition === "MATERIALIZED_OBJECT" ||
    item.outputAreas.some(
      (area) =>
        area.toLowerCase().includes("vlan") ||
        area.toLowerCase().includes("segment"),
    )
  ) {
  ...
  if (
    ...
    item.outputAreas.some(
      (area) =>
        area.toLowerCase().includes("policy") ||
        area.toLowerCase().includes("zone"),
    )
  ) {
    inputs.push(
      "designCore.securityPolicyFlow",
      "designCore.standardsAlignment",
    );
  }
  if (
    item.category === "cloud" ||
    ...
    item.outputAreas.some(
      (area) =>
        area.toLowerCase().includes("routing") ||
        area.toLowerCase().includes("wan"),
    )
  ) {
    inputs.push(
  ...
  if (item.category === "addressing")
    inputs.push("Engine2.enterpriseIpamReview", "validation.service");
  return Array.from(new Set(inputs));
}

function frontendConsumersFor(
  item: RequirementImpactRegistryItem,
  disposition: RequirementMaterializationDisposition,
) {
  const consumers = [
    "ProjectRequirementsPage",
    "ProjectOverviewPage.traceability",
  ];
  if (disposition === "MATERIALIZED_OBJECT")
    consumers.push("ProjectAddressingPage", "ProjectCoreModelPage");
  if (item.category === "security")
    consumers.push("ProjectValidationPage", "ProjectImplementationPage");
  if (item.category === "handoff") consumers.push("ProjectReportPage");
  return Array.from(new Set(consumers));
}

export function buildRequirementMaterializationPolicy(
  item: RequirementImpactRegistryItem,
): RequirementMaterializationPolicy {
  const disposition = policyDispositionFor(item);
  const objectTypes = objectTypesFor(item.key, disposition);
  const createdObjectTypes =
    disposition === "MATERIALIZED_OBJECT" ? objectTypes : [];
  ...
  return {
    ...
    affectedEngines: engineInputsFor(item, disposition).map((name) =>
      name
        .replace("designCore.", "")
        .replace("Engine1.", "Engine1:")
        .replace("Engine2.", "Engine2:"),
    ),
    ...
    noOpReason:
      "Requirement is absent, explicitly disabled, or not applicable; it must be preserved as explicit no-op evidence instead of silently disappearing.",
    reviewRequiredWhen: [
    ...
    unsupportedReason:
      disposition === "UNSUPPORTED"
        ? "No supported materialization policy exists for this requirement yet."
        : undefined,
    confidence: confidenceFor(disposition, item),
  };
}

export const REQUIREMENT_MATERIALIZATION_POLICIES: RequirementMaterializationPolicy[] =
  REQUIREMENT_IMPACT_REGISTRY.map(buildRequirementMaterializationPolicy);

function allVlanEvidence(evidence?: MaterializationEvidence) {
  const siteVlans = (evidence?.sites ?? []).flatMap((site) =>
    (site.vlans ?? []).map((vlan) => ({
      id: vlan.id,
  ...
  const addressingVlans = (evidence?.addressingRows ?? []).map((row) => ({
    ...
    vlanId: row.vlanId,
    cidr:
      row.canonicalSubnetCidr ?? row.sourceSubnetCidr ?? row.proposedSubnetCidr,
  }));
  return [...siteVlans, ...addressingVlans];
}

function evidenceForPolicy(
  policy: RequirementMaterializationPolicy,
  ...
  if (segmentFamilies.includes("SITE")) {
    for (const site of evidence?.sites ?? []) {
      ...
      actualEvidence.push(
        `Site ${site.name ?? site.siteCode ?? site.id ?? "unknown"} exists${site.defaultAddressBlock ? ` with block ${site.defaultAddressBlock}` : ""}.`,
      );
    }
  }
  ...
  for (const family of segmentFamilies.filter((family) => family !== "SITE")) {
    const matches = vlanRows.filter(
      (row) => row.vlanName === family || String(row.vlanName).includes(family),
    );
    for (const match of matches) {
      if (match.id) objectIds.push(match.id);
      actualEvidence.push(
        `${family} segment exists${match.vlanId ? ` as VLAN ${match.vlanId}` : ""}${match.cidr ? ` with ${match.cidr}` : ""}.`,
      );
    }
  }
  ...
  if (policy.createdObjectTypes.includes("DHCP scope candidate")) {
    ...
    if (dhcpCount > 0)
      actualEvidence.push(
        `${dhcpCount} DHCP scope row(s) exist for requirement-derived DHCP-capable segments.`,
      );
  }
...
const REVIEW_REQUIRED_WHEN_MISSING = new Set(["siteCount", "usersPerSite"]);

function statusFor(
  policy: RequirementMaterializationPolicy,
  active: boolean,
  evidence?: MaterializationEvidence,
): RequirementMaterializationStatus {
  if (!active && REVIEW_REQUIRED_WHEN_MISSING.has(policy.key))
    return "review-required";
  if (!active) return "explicit-no-op";
  if (policy.expectedDisposition === "UNSUPPORTED") return "unsupported";
  if (policy.expectedDisposition === "ENGINE_INPUT_SIGNAL")
    return "engine-input-signal";
  if (policy.expectedDisposition === "VALIDATION_BLOCKER")
...
export function buildRequirementMaterializationPolicySummary(
  ...
  const fieldOutcomes = REQUIREMENT_MATERIALIZATION_POLICIES.map((policy) => {
    const sourceValue =
      requirementValueToString(safeRequirements[policy.key]) || "not captured";
    const captured = Object.prototype.hasOwnProperty.call(
      safeRequirements,
      policy.key,
    );
    const active =
      captured && !isExplicitlyOffOrEmpty(safeRequirements[policy.key]);
    const actual = evidenceForPolicy(policy, evidence);
    const materializationStatus = statusFor(policy, active, evidence);
    const sourceType: RequirementSourceType = captured
      ? active
        ? "USER_PROVIDED"
        : "NOT_CAPTURED"
      : REVIEW_REQUIRED_WHEN_MISSING.has(policy.key)
        ? "REVIEW_REQUIRED"
        : "NOT_CAPTURED";
    const implementationBlocked =
      materializationStatus === "review-required" ||
    ...
    const reviewReason =
      materializationStatus === "review-required"
        ? REVIEW_REQUIRED_WHEN_MISSING.has(policy.key) && !active
          ? `${policy.key} was not captured as user intent; generated objects may only be review-required planning candidates.`
  ...
  const silentDropKeys = fieldOutcomes
    .filter(
      (item) =>
        item.captured &&
        item.active &&
        item.materializationStatus === "policy-missing",
    )
    .map((item) => item.key)
    .sort();
  ...
  return {
    ...
    materializedObjectCount: fieldOutcomes.filter(
      (item) => item.materializationStatus === "materialized",
    ).length,
    engineInputSignalCount: fieldOutcomes.filter(
      (item) => item.materializationStatus === "engine-input-signal",
    ).length,
    validationBlockerCount: fieldOutcomes.filter(
      (item) => item.materializationStatus === "validation-blocker",
    ).length,
    reviewItemCount: fieldOutcomes.filter(
      (item) => item.materializationStatus === "review-required",
    ).length,
    explicitNoOpCount: fieldOutcomes.filter(
      (item) => item.materializationStatus === "explicit-no-op",
    ).length,
    unsupportedCount: fieldOutcomes.filter(
      (item) => item.materializationStatus === "unsupported",
    ).length,
    policyMissingCount: fieldOutcomes.filter(
      (item) => item.materializationStatus === "policy-missing",
    ).length,
    ...
    notes: [
      ...
      "Missing siteCount or usersPerSite is not treated as selected default intent; it remains review-required until confirmed by the user.",
...
export function parseRequirementsForMaterializationPolicy(
