// V1 security policy flow control markers: V1PolicyState, consequenceSummary, reviewReason, SECURITY_NAT_REQUIRED_FLOW_UNCOVERED, SECURITY_RULE_SHADOWED_BY_EARLIER_RULE, SECURITY_IMPLICIT_DENY_NOT_MODELED.
import type {
  NatRule,
...
function generatedDefaultDenyRuleId(sourceZone: SecurityZone, destinationZone: SecurityZone) {
  return `policy-baseline-deny-${normalizeIdentifierSegment(sourceZone.id)}-to-${normalizeIdentifierSegment(destinationZone.id)}`;
}

function completeBaselineDefaultDenyPolicyRules(params: {
  securityZones: SecurityZone[];
  policyRules: SequencedPolicyRule[];
}): SequencedPolicyRule[] {
  const completed = [...params.policyRules];
  const existingDenyPairs = new Set(
    completed
      .filter((rule) => rule.action === "deny")
      .map((rule) => zonePairId(rule.sourceZoneId, rule.destinationZoneId)),
  );
  const maxSequence = completed.reduce((highest, rule) => Math.max(highest, rule.sequence), 0);
  ...
  for (const sourceZone of params.securityZones) {
    for (const destinationZone of params.securityZones) {
      if (!implicitDenyExpected(sourceZone, destinationZone)) continue;
      const pairId = zonePairId(sourceZone.id, destinationZone.id);
      if (existingDenyPairs.has(pairId)) continue;

      completed.push({
        id: generatedDefaultDenyRuleId(sourceZone, destinationZone),
        name: `Baseline deny ${sourceZone.name} to ${destinationZone.name}`,
        sourceZoneId: sourceZone.id,
        destinationZoneId: destinationZone.id,
        action: "deny",
        services: ["any"],
        truthState: "review-required",
        rationale: "V1 wizard/security-policy matrix completion requires explicit default-deny evidence for every high-risk zone pair. This is neutral planning evidence, not a vendor command or approval.",
        notes: [
          "Generated by V1 security-policy matrix completion so wizard-created zones do not fail because default-deny evidence was omitted downstream.",
          "Implementation remains review-required until real firewall zones, interfaces, logging, rule order, and change approval are confirmed.",
        ],
        sequence: nextSequence,
      });
      existingDenyPairs.add(pairId);
      nextSequence += 10;
    }
  }

  return completed.sort((left, right) => left.sequence - right.sequence || left.id.localeCompare(right.id));
}
...
function flowInputFromSegmentationExpectation(expectation: SegmentationFlowExpectation, securityZones: SecurityZone[]): BaseFlowRequirementInput | null {
  ...
  return {
    ...
    notes: [
      "Derived from the V1 segmentation expectation so security policy coverage can be evaluated as a first-class flow requirement.",
    ],
  };
}
...
function buildAdditionalFlowInputs(model: SecurityPolicyNetworkObjectModel): BaseFlowRequirementInput[] {
  ...
  const pushV1DenyFlow = (id: string, name: string, sourceZone: SecurityZone | undefined, destinationZone: SecurityZone | undefined, rationale: string) => {
    ...
    flowInputs.push({
      ...
      rationale,
      truthState: "proposed",
      notes: ["V1 explicit default-deny policy reconciliation flow."],
    });
  };
  ...
  pushV1DenyFlow("security-flow-V1-internal-users-to-management-deny", "General internal users must not administer the management plane", internalZone, managementZone, "Normal corporate user networks must not broadly reach device administration; scoped admin/operations access belongs in separate review flows.");
  pushV1DenyFlow("security-flow-V1-guest-to-dmz-deny", "Guest to DMZ must be denied by default", guestZone, dmzZone, "Guest networks must remain internet-only and isolated from published service or remote-access edge segments.");
  pushV1DenyFlow("security-flow-V1-general-wan-to-dmz-deny", "General WAN to DMZ must be denied by default", wideAreaNetworkZone, dmzZone, "General inbound WAN traffic is denied by default; approved published services use separate explicit review/allow flows.");

  pushV1DenyFlow("security-flow-V1-guest-to-voice-deny", "Guest to voice services must be denied by default", guestZone, voiceZone, "Guest networks must not reach voice services or call-control paths by default.");
  pushV1DenyFlow("security-flow-V1-voice-to-management-deny", "Voice services to management plane must be denied by default", voiceZone, managementZone, "Voice endpoints and call-control services must not administer network-management services.");
  pushV1DenyFlow("security-flow-V1-wan-to-voice-deny", "WAN to voice services must be denied by default", wideAreaNetworkZone, voiceZone, "Untrusted WAN sources must not reach voice services unless a reviewed voice edge/SBC design exists.");
  pushV1DenyFlow("security-flow-V1-guest-to-iot-deny", "Guest to IoT must be denied by default", guestZone, iotZone, "Guest networks must remain isolated from shared device networks.");
  pushV1DenyFlow("security-flow-V1-guest-to-transit-deny", "Guest to WAN transit must be denied by default", guestZone, transitZone, "Guest networks should only use reviewed internet egress, not direct transit access.");
  pushV1DenyFlow("security-flow-V1-wan-to-management-deny", "WAN to management must be denied by default", wideAreaNetworkZone, managementZone, "Untrusted WAN sources must not reach management-plane services.");
  pushV1DenyFlow("security-flow-V1-wan-to-guest-deny", "WAN to guest must be denied by default", wideAreaNetworkZone, guestZone, "Guest networks are not inbound WAN destinations.");
  pushV1DenyFlow("security-flow-V1-wan-to-iot-deny", "WAN to IoT must be denied by default", wideAreaNetworkZone, iotZone, "Shared device networks must not be externally reachable.");
  pushV1DenyFlow("security-flow-V1-wan-to-transit-deny", "WAN to WAN-transit internals must be denied by default", wideAreaNetworkZone, transitZone, "Transit segments must not accept broad inbound WAN traffic.");
  pushV1DenyFlow("security-flow-V1-dmz-to-internal-deny", "DMZ to internal must be denied by default", dmzZone, internalZone, "DMZ-to-internal access requires exact application exceptions, not broad reachability.");
  pushV1DenyFlow("security-flow-V1-dmz-to-management-deny", "DMZ to management must be denied by default", dmzZone, managementZone, "DMZ workloads must not pivot into management-plane services.");
  pushV1DenyFlow("security-flow-V1-iot-to-management-deny", "IoT to management must be denied by default", iotZone, managementZone, "Shared device networks must not reach network administration services.");
  pushV1DenyFlow("security-flow-V1-transit-to-management-deny", "WAN transit to management must be denied by default", transitZone, managementZone, "WAN/cloud transit segments must not inherit management-plane access.");
...
export function buildSecurityPolicyFlowModel(params: {
  ...
  const inputPolicyRules = networkObjectModel.policyRules.map<SequencedPolicyRule>((rule, index) => ({ ...rule, sequence: index + 10 }));
  const policyRules = completeBaselineDefaultDenyPolicyRules({
    securityZones: networkObjectModel.securityZones,
    policyRules: inputPolicyRules,
  });
  ...
  const serviceGroups = buildServiceGroups(dedupedFlowInputs, policyRules);
  const serviceObjects = buildServiceObjects(dedupedFlowInputs, policyRules, serviceGroups);
