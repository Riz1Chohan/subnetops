...
function buildSectionGates(params: {
  ...
  const checks: Record<string, { readiness: V1Readiness; evidence: string[]; blockers: string[]; truthLabels: V1ReportSectionGateRow["truthLabels"] }> = {
    ...
    "validation-findings": { readiness: readinessFromBackend(params.V1ValidationReadiness.overallReadiness), evidence: [`Validation authority readiness ${params.V1ValidationReadiness.overallReadiness}; ${params.V1ValidationReadiness.blockingFindingCount} blocking finding(s); ${params.V1ValidationReadiness.rootBlockerCount ?? 0} root blocker(s).`], blockers: params.V1ValidationReadiness.findings?.filter((f: any) => f.category === "BLOCKING" && (f.findingClass === "ROOT_BLOCKER" || !f.findingClass)).map((f: any) => f.title) ?? [], truthLabels: ["BACKEND_COMPUTED", "BLOCKED", "REVIEW_REQUIRED"] },
...
export function buildV1ReportExportTruthControl(params: {
  ...
  const evidenceView = buildReportEvidenceView({
    ...
    V1EnterpriseIpamTruth: params.V1EnterpriseIpamTruth,
    V1ImplementationPlanning: params.V1ImplementationPlanning,
    V1ImplementationTemplates: params.V1ImplementationTemplates,
    reportTruth: params.reportTruth,
    diagramTruth: params.diagramTruth,
    networkObjectModel: params.networkObjectModel,
    reportExportReadiness: overallReadiness,
    omittedEvidenceSummaries,
  });
  const exportConsistencyProof = buildRuntimeExportConsistencyProof({
    designCore: {
