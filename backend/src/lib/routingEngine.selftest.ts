...
const multiSiteFixture = {
  id: "project-V1-routing-rich",
  name: "V1 Routing Rich Fixture",
  basePrivateRange: "11.0.0.0/16",
  ...
  sites: [
    {
      ...
      name: "HQ",
      siteCode: "HQ",
      defaultAddressBlock: "11.0.0.0/22",
      vlans: [
        {
          id: "vlan-hq-users",
          vlanId: 10,
...
const noTransitFixture = {
  ...multiSiteFixture,
  id: "project-V1-no-transit",
  name: "V1 No Transit Fixture",
  ...
  sites: [
    {
      ...
      name: "HQ",
      siteCode: "HQ",
      defaultAddressBlock: "11.0.0.0/24",
      vlans: [
        {
          id: "vlan-hq-users",
          vlanId: 10,
...
run("V1 generates backend route table entries with neutral administrative distance and longest-prefix metadata", () => {
...
run("V1 validates static and default route next-hop objects against backend network objects", () => {
  const snapshot = snapshotFor(multiSiteFixture);
  const routing = snapshot.networkObjectModel.routingSegmentation;
...
run("V1 builds a bidirectional site-to-site reachability matrix", () => {
  const snapshot = snapshotFor(multiSiteFixture);
  const routing = snapshot.networkObjectModel.routingSegmentation;
...
run("V1 detects route duplicates overlaps and missing next-hop objects in direct backend routing model input", () => {
  const routing = buildRoutingSegmentationModel({
    ...
    networkObjectModel: {
      routeDomains: [
        {
          ...
          subnetCidrs: ["11.0.0.0/24", "10.92.1.0/24"],
      ...
      securityZones: [
        { id: "security-zone-internal", name: "Internal", zoneRole: "internal", truthState: "inferred", siteIds: ["site-hq", "site-branch"], vlanIds: [10], subnetCidrs: ["11.0.0.0/24", "10.92.1.0/24"], routeDomainId: "route-domain-corporate", isolationExpectation: "restricted", notes: [] },
      ...
      interfaces: [
        { id: "interface-hq-users", name: "Vlan10", deviceId: "device-site-hq-layer3-gateway", siteId: "site-hq", interfaceRole: "vlan-gateway", truthState: "configured", subnetCidr: "11.0.0.0/24", routeDomainId: "route-domain-corporate", notes: [] },
    ...
    siteSummaries: [
      { siteId: "site-hq", siteName: "HQ", siteCode: "HQ", currentSiteBlock: "11.0.0.0/24", minimumRequiredSummary: "11.0.0.0/24", status: "good", coveredSubnetCount: 1, notes: [] },
...
run("V1 blocks multi-site reachability claims when transit and return paths are not modeled", () => {
...
console.log("\nV1 backend routing engine self-test complete.");
