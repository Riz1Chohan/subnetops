...
const richSecurityFixture = {
  id: "project-V1-security-rich",
  name: "V1 Security Rich Fixture",
...
function snapshotFor(): Snapshot {
  ...
  if (!snapshot) throw new Error("V1 fixture did not produce a design-core snapshot.");
  return snapshot;
}

run("V1 builds a backend security policy matrix instead of frontend policy assumptions", () => {
...
run("V1 detects implicit deny gaps for high-risk zone pairs", () => {
  const snapshot = snapshotFor();
  const security = snapshot.networkObjectModel.securityPolicyFlow;
...
run("V1 detects rule order shadowing and broad allow risk", () => {
  const zoneGuest = {
    id: "zone-guest",
    name: "Guest",
    zoneRole: "guest",
...
run("V1 reviews NAT coverage for NAT-required egress flows", () => {
  const snapshot = snapshotFor();
  const security = snapshot.networkObjectModel.securityPolicyFlow;
...
run("V1 treats required NAT as ready when zones translation and flow coverage are valid", () => {
  const zoneInternal = {
    id: "zone-internal",
    name: "Internal",
    zoneRole: "internal",
...
run("V1 does not let allow-only rules satisfy default-deny boundaries", () => {
  const zoneGuest = {
    id: "zone-guest",
    name: "Guest",
    zoneRole: "guest",
...
run("V1 exposes service groups logging requirements and observed first-match rule evidence", () => {
  const snapshot = snapshotFor();
  const security = snapshot.networkObjectModel.securityPolicyFlow;
...
run("V1 keeps security outputs vendor-neutral without command generation", () => {
  const snapshot = snapshotFor();
  const security = snapshot.networkObjectModel.securityPolicyFlow;
  const allNotes = [
