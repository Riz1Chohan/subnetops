...
function assertVendorNeutralModel(snapshot: Snapshot, scenarioId: string) {
  ...
  assert.equal(model.summary.commandGenerationAllowed, false, `${scenarioId}: command generation must remain disabled in V1`);
  assert.equal(model.summary.vendorSpecificCommandCount, 0, `${scenarioId}: V1 must not emit vendor-specific commands`);
  ...
  for (const template of model.templates) {
    ...
    assert.ok(template.commandGenerationReason.toLowerCase().includes("later gated stage"), `${scenarioId}: ${template.id} must explain why commands are deferred`);
  ...
  for (const leak of commandLeakWords) {
    assert.equal(serializedTemplates.includes(leak), false, `${scenarioId}: V1 template leaked vendor command-like text: ${leak}`);
  }
}

run("V1 templates are generated from the scenario regression matrix", () => {
  assert.ok(V1_SCENARIOS.length >= 8, "V1 should reuse the V1 scenario library instead of testing one toy input.");
});

for (const scenario of V1_SCENARIOS) {
  run(`V1 vendor-neutral templates: ${scenario.id}`, () => {
...
run("V1 keeps command generation gated", () => {
  const snapshots = V1_SCENARIOS.map((scenario) => buildDesignCoreSnapshot(scenario.project as never)).filter((snapshot): snapshot is Snapshot => Boolean(snapshot));
  ...
  assert.ok(totalTemplates > 0, "V1 must produce real template artifacts from scenarios.");
  assert.equal(totalVendorCommands, 0, "V1 must not generate vendor-specific command artifacts.");
