...
run("V1 adds engineering metadata to every implementation step", () => {
...
run("V1 models real dependencies instead of a flat checklist", () => {
  const plan = implementationPlanFixture();
...
run("V1B compiles graph-driven dependencies for security and NAT steps", () => {
  const plan = implementationPlanFixture();
...
run("V1 treats uncovered NAT-required flows as implementation blockers", () => {
  const plan = implementationPlanFixture();
  const flowStep = plan.steps.find((step) => step.targetObjectId === securityFlowId);
  const natStep = plan.steps.find((step) => step.targetObjectId === natRuleId);
...
run("V1 exposes evidence, rollback, and blast-radius summary proof", () => {
  const plan = implementationPlanFixture();
...
run("V1C creates device operational-safety gates", () => {
  const plan = implementationPlanFixture();
...
run("V1C gates risky route security and NAT steps on operational safety", () => {
  const plan = implementationPlanFixture();
...
run("V1D generates object-level and flow-level verification matrix checks", () => {
  const plan = implementationPlanFixture();
...
run("V1D keeps route missing next-hop as a route-step blocker", () => {
...
run("V1D keeps DHCP without a matching gateway interface blocked", () => {
  const plan = implementationPlanFixture((model) => {
...
run("V1D keeps missing security policy as a security-step blocker", () => {
  const plan = implementationPlanFixture((model) => {
...
run("V1D propagates upstream routing ERROR findings into affected implementation steps", () => {
  const plan = implementationPlanFixture((model) => {
...
run("V1D allows valid covered NAT to avoid blocked readiness", () => {
  const plan = implementationPlanFixture((model) => {
...
run("V1D turns upstream security WARNING into affected-step review metadata", () => {
  const plan = implementationPlanFixture((model) => {
    model.devices[0].managementIp = "10.36.99.10";
    model.devices[0].notes = [
      "management access verified",
