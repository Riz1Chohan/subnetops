...
run("free range calculation clips outside ranges before reporting gaps", () => {
  const ranges = calculateFreeRanges(parseCidr("10.90.0.0/24"), [
    { start: parseCidr("10.89.255.0/24").network, end: parseCidr("10.90.0.63/26").broadcast },
  ]);

  assert.equal(ranges.length, 1);
  assert.equal(ranges[0]?.rangeSummary, "10.90.0.64 - 10.90.0.255");
});
