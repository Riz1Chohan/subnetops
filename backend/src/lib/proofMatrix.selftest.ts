...
run("overlap detection distinguishes containment, adjacency, and separation", () => {
  assert.equal(cidrsOverlap(parseCidr("10.0.0.0/23"), parseCidr("11.0.0.0/24")), true);
  assert.equal(cidrsOverlap(parseCidr("10.0.0.0/24"), parseCidr("11.0.0.0/24")), false);
...
run("used-range normalization merges overlaps and adjacencies consistently", () => {
  ...
  assert.deepEqual(normalized, [
    ...
    { start: parseCidr("11.0.0.0/26").network, end: parseCidr("11.0.0.0/26").broadcast },
  ]);
});
