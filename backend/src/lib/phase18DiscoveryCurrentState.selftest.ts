...
import {
  ...
  buildPhase18DiscoveryCurrentStateControl,
} from "../services/designCore/designCore.phase18DiscoveryCurrentStateControl.js";
...
const result = buildPhase18DiscoveryCurrentStateControl({
  project: {
...
assert.equal(result.contract, PHASE18_DISCOVERY_CURRENT_STATE_CONTRACT);
...
assert(result.findings.some((finding) => finding.code === "PHASE18_DISCOVERY_CONFLICT_REQUIRES_RECONCILIATION"));
assert(result.proofBoundary.some((line) => line.includes("does not perform live network discovery")));

const emptyResult = buildPhase18DiscoveryCurrentStateControl({
  project: {
...
assert(emptyResult.findings.some((finding) => finding.code === "PHASE18_MANUAL_DISCOVERY_EMPTY_STATE"));
console.log("[phase18] Discovery/current-state selftest passed");
