import { buildV1StandardsRulebookControl, V1_STANDARDS_ALIGNMENT_RULEBOOK_CONTRACT } from "../services/designCore/designCore.standardsRulebookControl.js";
import type { NetworkObjectModel, V1RequirementsClosureControlSummary, StandardsAlignmentSummary } from "../services/designCore.types.js";
...
const summary = buildV1StandardsRulebookControl({
...
assert(summary.contractVersion === V1_STANDARDS_ALIGNMENT_RULEBOOK_CONTRACT, "contract marker mismatch");
...
assert(summary.ruleRows.every((row) => row.applicabilityCondition && row.remediationGuidance && row.exceptionPolicy && row.requirementRelationships.length >= 0), "rules must carry V1 governance fields");

console.log("[V1] Standards rulebook control selftest passed");
