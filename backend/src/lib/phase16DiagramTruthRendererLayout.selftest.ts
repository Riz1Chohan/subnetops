...
import { buildPhase16DiagramTruthControl, PHASE16_DIAGRAM_TRUTH_RENDERER_LAYOUT_CONTRACT } from "../services/designCore/designCore.phase16DiagramTruthControl.js";
...
const diagramTruth: any = {
  ...
  renderModel: {
    summary: {
      ...
      layoutMode: "phase16-backend-truth-layout-contract",
...
const phase15ReportExportTruth: any = { overallReadiness: "REVIEW_REQUIRED" };
const result = buildPhase16DiagramTruthControl({ diagramTruth, networkObjectModel, phase15ReportExportTruth });

assert.equal(result.contract, PHASE16_DIAGRAM_TRUTH_RENDERER_LAYOUT_CONTRACT);
...
assert.equal(result.findings[0].code, "PHASE16_DIAGRAM_TRUTH_RENDERER_LAYOUT_CONTROLLED");
console.log("[phase16] Diagram truth/render/layout selftest passed");
