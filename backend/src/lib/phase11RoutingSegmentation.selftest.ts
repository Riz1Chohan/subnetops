...
import { buildPhase11RoutingSegmentationControl } from "../services/designCore/designCore.phase11RoutingSegmentationControl.js";
import type { NetworkObjectModel, Phase10DesignGraphControlSummary } from "../services/designCore.types.js";
...
const phase10 = { objectCoverage: [{ objectId: "rd-default", objectType: "route-domain", dependencyState: "CONNECTED" }] } as unknown as Phase10DesignGraphControlSummary;
const result = buildPhase11RoutingSegmentationControl({ project: { sites: [{ id: "site-hq", name: "HQ" }, { id: "site-branch", name: "Branch" }], requirementsJson: { siteCount: 2, dualIsp: true, cloudConnected: true, guestWifi: true, guestPolicy: "internet-only isolated guest access" } }, networkObjectModel, phase10DesignGraph: phase10 });
assert.equal(result.contract, "PHASE11_ROUTING_SEGMENTATION_PROTOCOL_AWARE_PLANNING_CONTRACT");
...
assert.ok(result.findings.some((finding) => finding.code === "PHASE11_ROUTING_SIMULATION_UNAVAILABLE"));
console.log("[phase11] Routing segmentation protocol-aware planning selftest passed");
