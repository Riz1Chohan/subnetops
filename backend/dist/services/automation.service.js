...
async function queueAutomatedOverdueReminders() {
    ...
    for (const [userId, userTasks] of groups) {
        ...
        await queueEmail({
            ...
            payload: { count: userTasks.length, tasks: userTasks.map((task) => ({ id: task.id, body: task.body, project: task.project.name, dueDate: task.dueDate, priority: task.priority })) },
        });
        await prisma.projectComment.updateMany({
            where: { id: { in: userTasks.map((task) => task.id) } },
...
async function queueAutomatedDigests() {
    ...
    for (const pref of prefs) {
        ...
        if (pref.lastDigestQueuedAt && Date.now() - new Date(pref.lastDigestQueuedAt).getTime() < minAge)
            continue;
        const tasks = await prisma.projectComment.findMany({
        ...
        const overdue = tasks.filter((task) => task.dueDate && new Date(task.dueDate).getTime() < Date.now()).length;
        await queueEmail({
            toEmail: pref.user.email,
            subject: `SubnetOps ${pref.digestFrequency.toLowerCase()} digest`,
            templateKey: "automated-task-digest",
            payload: { frequency: pref.digestFrequency, open: tasks.length, overdue, tasks: tasks.map((task) => ({ body: task.body, project: task.project?.name, dueDate: task.dueDate, priority: task.priority })) },
        });
        await createNotification({
            userId: pref.userId,
            type: "SYSTEM",
